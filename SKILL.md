...
6. **铁律 6·审查不可代**：第八步断后校验**必须由独立审查智能体执行**——通过 Agent 工具启动的 subagent 与解卦主流程上下文隔离，只依据卦例目录中的步骤 md 与 JSON 证据独立审查。**禁止执行者自查代替**（自检报告存在"不诚实标记"造假风险，正是本步改制的动因）。发现问题必须返回对应步骤修正，修正后重新启动审查复审，直至通过（复审上限 3 轮）。
...
> 💡 **本步分工（铁律 6·审查不可代）**：审查必须由**独立审查智能体**执行——通过 Agent 工具启动 subagent（general-purpose 类型，与主流程同款模型），其上下文与解卦主流程完全隔离，只依据卦例目录落地文件判断。主流程只负责**委派 → 接收报告 → 按报告修正 → 复审**，**禁止主流程自己跑 A/B/C/D 自查代替审查**。自检报告"运动员兼裁判"的造假风险（旧版第八步最终检查点"16条有无不诚实标记"正是自认软肋）是本步改制的动因。
...
- ⛔ 逆向思维是硬性要求：只顺着报告找印证、不复盘反证、未执行逆向思维检查 → 审查报告无效，须重新委派审查
...
1. 主流程收到审查智能体的报告与最终结论：
   - 「通过」或「存在X项问题已全部修正可交付」→ 进入第九步
   - 存在未修正 ⚠️ 项 → 按报告问题清单回到对应步骤修正（修正必须落到步骤 md 文件中），修正后**重新启动审查智能体复审**
2. 复审上限 **3 轮**。第 3 轮仍未通过：以最后一轮审查报告为准交付，并在报告中显式标注「三轮复审未通过，按最后一轮报告交付」，问题清单原样带入最终报告（避免无限循环，不掩盖问题）。
...
- 复审轮次：[第N轮 / 三轮未通过按最后一轮交付]
- 修正记录：[如有多轮，逐轮列出：问题 → 修正的步骤 → 修正内容]
```

> 📁 落地：`{卦例目录}/步骤8-断后校验.md` 由**审查智能体**写出（主流程不得代写、不得改写报告内容）。报告含 A/B/C/D 四项与最终结论，是第九步的唯一准入依据。

**✅ 最终检查点**：是否确实委派了独立审查智能体（而非自查）？审查报告已由审查智能体落地？最终结论为「通过」或「全部修正可交付」（或已显式标注三轮未通过）？⚠️ 项是否已实际修正并复审确认？

---

### 第九步：生成 HTML 报告

**⛔ 准入条件**：第八步智能体审查已**通过**（审查报告最终结论为「通过」或「存在X项问题已全部修正可交付」；三轮复审仍未通过时，以最后一轮报告为准交付并显式标注）。若审查未通过（存在未修正的⚠️），**禁止执行此步**，必须先回到对应步骤修正并重新启动审查复审。
