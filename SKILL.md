...
> | **Tier C·第四步前预读** | 进入第四步前 | 该 intent 路由表行中所有 Tier C 标记的项（frv 版六神类象由第五步 `jie-gua-xiang-jie.md` 第五节「六神吉凶象表」承担） | 六神类象断语 |
...
> 🔌 **Tier C 加载触发**（铁律 14 执行点）：进入第四步前，按路由表加载本 intent 的 Tier C 标记项（若存在）。六神类象基础由第五步 `jie-gua-xiang-jie.md` 第五节「六神吉凶象表」承担。本步开头清单需列明"本步新增加载的 Tier C 项"。
...
> **⛔ 强制加载**：**必须**加载 `references/jie-gua-xiang-jie.md` 第五节（六神吉凶象表）。**若 intent 为「失物」「寻人」「天气」「健康」「阳宅」 → 必须额外加载 `references/bagua-leixiang.md` 和 `references/shier-dizhi-leixiang.md` 全文**，不得凭记忆取象：
