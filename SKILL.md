...
6. **铁律 6·审查不可代**：第八步断后校验**必须由独立审查智能体执行**——通过 Agent 工具启动的 subagent 与解卦主流程上下文隔离，只依据卦例目录中的步骤 md 与 JSON 证据独立审查。**禁止执行者自查代替**（自检报告存在"不诚实标记"造假风险，正是本步改制的动因）。**单轮审查制（v1.7.5 简化）**：审查智能体只执行**一轮**——发现问题即如实写入报告，主流程按报告问题清单回退对应步骤修正（修正必须落到步骤 md 文件），修正后**不再启动复审**，直接进入第九步；修正内容与未通过项在步骤8报告中显式记录，不掩盖问题（实测：多轮复审边际收益低、消耗大，一轮审查 + 主流程落盘修正已足够）。
...
11. **铁律 11·格局不遗漏**：第三步必须检测三会局、六合、六冲、伏吟、反吟、六冲卦等特殊格局。第七步综合断语必须**逐一分析**每个检测出的特殊格局并纳入总体判断——四要素缺一不可：①成局/虚势状态 ②所强/所制之爻（六亲视角）③对用神吉凶 ④结果断语。禁止「只列不析」：如三会局只写「巳午未三支俱全，已成三会之势」而不交代会出何局、加强何爻、对用神如何，即不合格，审查必判漏判。JSON step3.pattern 字段必须包含完整格局分析，不得用一句「静卦无特殊格局」敷衍。
...
> | **Tier C·第四步前预读** | 进入第四步前 | 该 intent 路由表行中所有 Tier C 标记的项（frv 版六神兽类象由第五步 `jie-gua-xiang-jie.md` 第五节「六神兽吉凶象表」承担） | 六神兽类象断语 |
...
>
> 示例：
> - "老婆的病会影响胎儿吗" → 主 intent=健康（关注病），用神=子孙（健康+孕产都取子孙），Tier B 预读=`wuxing-shengke 第七节旺相休囚死`，Tier D 预读=`bagua-leixiang 全文` + `shier-dizhi-leixiang 全文`（健康 intent 人体定位病灶）
...
| 孕产 | `wuxing-shengke.md` |
| 出行 | `yingqi-faze.md` |
| 失物 | `bagua-leixiang.md` 全文 + `shier-dizhi-leixiang.md` 全文 + `yingqi-faze.md` |
| 寻人 | `bagua-leixiang.md` 全文 + `shier-dizhi-leixiang.md` 全文 + `yingqi-faze.md` |
| 阳宅 | `bagua-leixiang.md`（方位）+ `64-gua.md`（卦宫属性） |
...
- 待加载 Tier 提醒：
  ...
  - Tier C（第四步前）：[如 liuqin 第三节~第七节六神兽部分]
...
特殊格局（引用 JSON patterns 字段，不得手动重新识别）：
  - 三会局：[读 patterns.sanhui_ju，空则写"无"]；注明会出[五行]局（对本宫为[六亲]局）、所强之爻与对用神吉凶（铁律 11）
...
**✅ 检查点**：是否遍历了所有 dong:true 的爻？🚨 **每个动爻的五行生克方向是否经过「本爻五行→变爻五行→查表→判定」四步显式确认？**（如申=金、卯=木 → 金克木 → 本爻克变爻 = 化出我克者，非回头克）每爻是否判定了化进/退/十二长生？变化结论是否从受控词表中取值？多爻联动是否按 合>冲>刑>害>生>克 优先级审视？暗动是否严格区分了日破？生克链条是否画到变爻？**特殊格局是否引用了 JSON `patterns` 字段（而非手动重新识别）？**
...
> 🔌 **Tier C 加载触发**（铁律 14 执行点）：进入第四步前，按路由表加载本 intent 的 Tier C 标记项（若存在）。六神兽类象基础由第五步 `jie-gua-xiang-jie.md` 第五节「六神兽吉凶象表」承担。本步开头清单需列明"本步新增加载的 Tier C 项"。
...
> **⛔ 强制加载**：**必须**加载 `references/jie-gua-xiang-jie.md` 第五节（六神兽吉凶象表）。**若 intent 为「失物」「寻人」「天气」「健康」「阳宅」 → 必须额外加载 `references/bagua-leixiang.md` 和 `references/shier-dizhi-leixiang.md` 全文**，不得凭记忆取象：
...
> 🔌 **Tier E 加载触发**（铁律 14 执行点）：进入第七步前必须加载 Tier E references——`te-shu-ge-ju.md`（感情 intent 或涉及六合六冲时）。本步开头清单需列明"本步新增加载的 Tier E 项"。

> **⛔ 强制加载**：**必须**加载 `references/jie-gua-xiang-jie.md` 第七节（完整输出模板 + 填写要求）。把场景结论填入断语模板，不得跳过。
...
**✅ 检查点**：模板中有无未填的 [ ]？每条结论能否回溯到前六步输出？定性是否与旺衰+动变一致？凶兆是否指出救应？**特殊格局是否逐一完成四要素分析并纳入总体判断（成局状态/所强之爻/对用神吉凶/结果，铁律 11）**？语气是否符合半文半白？
...
> ⛔ **强制加载**：主流程启动审查智能体前，先加载 `references/jie-gua-xiang-jie.md` 第八节（步骤完整性检查表 + 交叉验证规则 + 16条原则映射表），确认审查输入物齐备，并熟读下方审查 prompt 模板。
...
2. **三合/三会局（第三步 + 第七步）**：JSON patterns 中三合/三会局（及缺字半合、日月补字、旬空打折）是否被第七步按铁律 11 四要素**逐一分析**（成局/虚势状态、所强所制之爻、对用神吉凶、结果断语），只引不析=漏判；报告自称「无某局」→ 回查 JSON 是否真为空；局际方向（局与局/局与用神的生克）是否验算
...
5. 警惕「套话掩盖未分析」：「力量增强」「势不可挡」「助力明显」等形容词必须能对应到具体爻与六亲，否则视为未分析
...
- 应期覆盖完整性：空亡必须同时列出填实日与冲空日，休囚列出当令月与生扶月，安静列出逢冲日，伏藏列出值伏日与冲飞日；只列一条 → 判⚠️漏判
- 格局↔断语完整性（铁律 11）：paipan JSON patterns 中每个三会/三合/六合/六冲/伏吟/反吟等格局，第七步综合断语必须逐一交代四要素（成局/虚势状态、所强所制之爻、对用神吉凶、结果断语）；只提及不分析（如三会局只写「已成三会之势」未分析会出何局、加强何爻、对用神如何）→ 判⚠️漏判
...
4. **写入后立即校验 JSON schema 完整性**（必须执行，不可跳过）：
   ...
   ```
   > ⛔ 校验逻辑统一收敛到 `generate_report.py --validate` 子命令，**禁止在 SKILL.md 中写 inline Python heredoc**（跨平台引号转义易出错，且 schema 演进要改两处）。`--validate` 模式检查 `meta`/`yao`/`steps` 顶层域 + `step1`~`step8` 齐全 + `step5` 四个六神兽字段 + 每爻 `ben_yin_yang` + **🚨 占位内容检测**（若 step1.note 含"未做解卦分析"、step7.final_verdict 含"未做解卦分析"等占位文本 → 报错阻断）+ **🚨 md_full 全文保真检测**（v1.7.1：缺失/为空/长度低于下限/含占位文本 → 报错阻断）。
