# Changelog

## 当前维护重点索引

- **版本同步**：`SKILL.md metadata.version` 是唯一权威；同步 README 当前版本、CHANGELOG 顶部版本、HTML guide 脚本版本、`generate_report.py` 文件头和 CLI description。
- **格局检测**：重点回看 v1.5.7、v1.6.1、v1.7.2、v1.8.1；格局以 `paipan.py detect_patterns()` 输出的 JSON `patterns` 为准。
- **HTML 空报告防护**：重点回看 v1.8.1、v1.8.4、v2.0.1；第九步必须从步骤 md 组装 `steps` 与 `md_full`。
- **reference 假读防护**：重点回看 v1.5.11、v1.6.0、v1.8.4；加载清单必须含关键句摘录。
- **编码问题**：重点回看 v1.5.3；Windows 验证先设置 `PYTHONIOENCODING=utf-8`。
- **应期分层校验**：重点回看 v1.8.2、v1.8.3、v1.8.4；第六步必须输出主应期、辅助节点、风险窗口三层。
- **规则编号检索**：见 `references/rule-index.md`；`TL-*` 为 SKILL.md 铁律，`PR-*` 为 `jie-gua-xiang-jie.md` 核心原则。

## v2.0.6 (2026-09-21)

### 六合卦纳支配对数据修正（同构 main v2.0.6 · 文档表述层）

> 起因：实占（丙午年丁酉月戊戌日，地火明夷变山火贲）中，变卦山火贲的六合纳支配对引发审查质疑；据此按 `scripts/paipan.py` 纳支实算八个六合卦对勘，发现六合卦「原理」列所记爻位与本卦真实纳支不符，导致多数配对错误。

**问题定位**：
- **卦名清单正确**——八个六合卦（天地否、山火贲、水泽节、雷地豫、火山旅、泽水困、地雷复、地天泰）按 `paipan.py` 纳支实算，初四/二五/三六三对**确实全部相合**；`paipan.py` 的检测本身无误。
- **错误仅在文档该表的「原理」列**——该列以「艮辰 离卯 → 辰酉合、午丑合」形式书写，而山火贲真实纳支为卯丑亥戌子寅（艮外卦＝戌子寅、离内卦＝卯丑亥），列中并无辰/酉/午三字，故配对错位。八行中六行有误，仅地雷复、地天泰两行正确。

**逐卦更正**（真实纳支 → 三对）：

| 卦 | 纳支（初→上） | 更正后三对 | 旧列错误 |
| --- | --- | --- | --- |
| 天地否 | 未巳卯午申戌 | 午未合、巳申合、卯戌合 | 1/3 错 |
| 山火贲 | 卯丑亥戌子寅 | 卯戌合、子丑合、寅亥合 | 2/3 错 |
| 水泽节 | 巳卯丑申戌子 | 巳申合、卯戌合、子丑合 | 2/3 错 |
| 雷地豫 | 未巳卯午申戌 | 午未合、巳申合、卯戌合 | 2/3 错 |
| 火山旅 | 辰午申酉未巳 | 辰酉合、午未合、巳申合 | 2/3 错 |
| 泽水困 | 寅辰午亥酉未 | 寅亥合、辰酉合、午未合 | 3/3 全错 |
| 地雷复 | 子寅辰丑亥酉 | 子丑合、寅亥合、辰酉合 | 正确 |
| 地天泰 | 子寅辰丑亥酉 | 子丑合、寅亥合、辰酉合 | 正确 |

**改动**：
- `references/64-gua.md`：六合卦表新增「纳支（初→上）」列，三对应合列按真实纳支重写；补「纳支来源」注与「旧版该列所记爻位与真实纳支不符」的更正说明。
- `references/te-shu-ge-ju.md`：「常见六合卦」表同步补纳支列与三对列（对齐 `64-gua.md`）。
- 版本同步：`SKILL.md`、README、CHANGELOG、`html-report-guide.md`、`generate_report.py` 一并升至 v2.0.6。
- **未改动**：`paipan.py`、`test_gua_patterns.py`。

## v2.0.5 (2026-09-19)

### 三合/三会成局口径校准（同构 main v2.0.5）

> 同构 main `3011600`。用户明定三合/三会成局条件，此前实现长期偏离，本次按口径重写状态机并同步 reference、SKILL.md 与测试。

**用户口径**：
- 三合局：(a) 三爻皆不发动 → 虚合，**必须日月来一个**方成局；(b) 日月都不入组 + 有一爻发动 → 成局；(c) 二字实、一字空亡 → 该空字**值日/值月时即等于日月入卦**（等于它发动了）→ 虚合待时，届时成局。
- 三会局：**同理**，但**多一个最高档——三爻全部发动＝严格成局**；**三合没有这个最高档**。

**改动**：
- `scripts/paipan.py`：抽出统一 `_activation_of()`（激活源 = 动爻 / 日月入组），两局共用。
  - 三会：四态 → **三态**（严格成局[三爻全动]／成局／三会之势）；⛔ 删除旧「2动+日月补第三字＝严格成局」与「三会」档；旬空不再作状态档。
  - 三合：判据修正为「有激活源即成局、无则虚合」；⛔ 明确**三合无「严格成局」档**。
  - 新增 `activation` 与 `void_trigger` 字段；`kong_positions` 语义收窄为「局力打折」。
- `references/di-zhi-relations.md`：第二、三节按口径重写（总纲段、a/b/c 三情形、空亡不作状态档、最高档之别）；认定报告模板增「激活源」行；区别速查表增「最高档」对比行；共同要点重排。
- `SKILL.md`：patterns 字段说明补 `activation`/`void_trigger` 与成局口径；第三步模板两行补「须注明状态与激活源」「三合无严格成局档」。
- `scripts/test_gua_patterns.py`：三会测试重写为三态 × 6 组合 + 最高档 + 空亡激活时机 + 缺支反例；三合测试新增「三字全动仍为成局（防最高档误套）」「日月不入组+1动→成局」「全静+空亡→虚合且标激活时机」三组用例。旧期望值已更新。
- 版本升至 v2.0.5（SKILL/README/html-report-guide/generate_report 四处同步）。

测试：`test_gua_patterns` 13 类全过；`check_release_consistency` 5 项 PASS。

## v2.0.4 (2026-09-19)

### 第零步交互改按需反问（软门禁）

> 同构 main v2.0.4。起因：会话记录显示自客户端 0.16.3 起，Agent 在自主模式下把「必须反问确认 intent」当成阻塞项，连续多卦直接自判排盘（"用户问题已完整，无需再交互"）。改为**按需反问**：只在信息真的不足或判断真的不确定时问，其余情况直接排盘——既不打断用户，又保住关键信息的准确性。

- `SKILL.md` 第零步第 1 条：**软门禁 A·时间范围**——取消"必须先问时间范围"的强制反问。仅在「答案周期可能跨月/跨年」的问题类型（何时能…、未来几年、长期发展，及事业/婚姻/置产/长期投资等长周期事项）且用户未指定时间时，反问一次；用户已给（"下个月""1-2 年""明年"）或短期事项则直接采用/自判。时间范围影响应期单位的选择倾向（短期以日/月、长期以年，见 `jie-gua-xiang-jie.md`「应期单位选择」）。反问无回应时按问题性质自判（长周期默认长期不限）并注明，**不阻塞流程**。
- `SKILL.md` 第零步第 2 条：**软门禁 B·意图把握**——判断有把握（关键词明确命中单一 intent、主用神唯一）时直接采用，不反问；不确定时（跨多个 intent、指向不明、主用神两可、题面含混）回读判断并请用户提出修改意见；无回应则按原判继续。
- `SKILL.md` 第零步第 3 条：sxtwl 检测改为分档——**已安装且两条软门禁均未触发 → 无需反问，直接进入 4b 孕产门禁**；已触发反问则并入同一轮；**未安装时的 A/B 选择仍必须发出**（此问不可省），并与软门禁问题合并为单条消息。
- `SKILL.md` 第零步第 4 条：新增无回应兜底——sxtwl 已安装按自判继续；未安装先按 B 回退，脚本报年份越界再回到 A。**4b 孕产门禁前置检查不受本改动影响**（仍必须先于建目录执行）。
- `SKILL.md` 第零步强制输出块：新增「时间范围」（含用户指定/已反问/自判及依据）与「意图把握」（有把握直接采用/已反问/无回应按原判）两行，随步骤 0 文件落盘，便于事后核查。
- `SKILL.md` 第零步检查点：改为核查软门禁是否**只在必要时**触发（把握足够时不得多问）、反问是否与 sxtwl 合并一轮、无回应是否已自判继续；保留 1/2 清理空目录、3 保留作废的原有处置。
- `SKILL.md` 铁律 12/14、第零步附路由表 Tier A 行与执行点：措辞由「意图确认」统一为「意图判定」，消除与新交互逻辑的内部矛盾。
- `README.md`：方式一改为反映新交互（两条软门禁说明 + 长周期未指定时间的示例对话），版本列表补 v2.0.4。
- 版本同步 v2.0.4：SKILL.md / README / CHANGELOG / html-report-guide / generate_report.py。

## v2.0.3 (2026-09-11)

### 审查修复与一句话手动排盘

> 本版本修复外部审查发现的 8 项问题（其中 4 项 frv 专属、4 项与 main 共有），并落地 README 承诺的「一句话手动排盘」。不削弱任何解卦规则。

- `scripts/paipan.py`：**移除误伤拦截词「生产」**——该词亦指生产项目/生产力/投产，会把求财、官运等正常问卦直接拦死（退出码 2）。拦截表仅保留无歧义的分娩类强特征词（分娩/临盆/临产/接生/产妇/生孩子等）。新增 `resolve_gua_name()`、`parse_dong_yao()`、`build_ygua_from_gua_name()`、`dong_yao_from_pair()` 与 CLI 参数 `--gua-name` / `--dong-yao` / `--bian-gua`：支持「卦名+动爻」或「本卦+变卦」一句话手动排盘，后者由两卦逐爻取差确定动爻，无需人工比爻；无实体掷币、不做立币判定，JSON `mode=manual`。支持八纯卦简称（「乾」）与带宫位写法（「坎-泽火革」）。**`ben_gua`/`bian_gua` 输出改为纯卦名**（去「宫-」前缀），宫位仍由 `ben_gua_gong` 保留。
- `scripts/generate_report.py`：同步移除拦截词「生产」（此为第二份门禁表，若漏改会在第九步把「生产项目」报告再次拦掉）；版本升至 v2.0.3。
- `SKILL.md`：内部门禁段删除「生产」并新增「禁用歧义特征词」说明；第零步新增 4b 孕产门禁前置检查（拦截发生在建目录之前，退出码 2 不再留空目录）；新增非 0 退出码统一处置表（1/2 清理空目录、3 保留作废记录、0 正常）；排盘命令块新增手动排盘三分支（`--dong-yao` / `--bian-gua` / 静卦）与 Agent 侧换算规程（换算全部交脚本，禁手算比爻）；「输入格式」补 `sanhe_ju`/`ben_gua_gong`/`shensha` 字段说明及 `ben_gua` 纯卦名口径；第零步 intent 速查表统一到 `jie-gua-xiang-jie.md` 权威口径（出行=应爻、词讼=世应、天气=官鬼/父母/妻财）；intent 路由表补「学业」「词讼」两行。
- `references/jie-gua-xiang-jie.md`：intent→用神映射表标注为唯一权威，出行行补「问出行方式加看父母」。
- `references/html-report-guide.md`：新增 2.5 节「`paipan_result.json` → `liuyao-data.json` 字段转换」——逐字段说明 `kong_wang`(list→string)、`date`(对象→字符串)、`kong_wang`→`special_tags`、`edition`、`shensha`（frv 不转换）等映射。
- `README.md`：方式二说明改为「卦名 + 动爻位置（或变卦名）」，删除未落地的「动爻二爻和五爻」示例、补 `--bian-gua` 说明；步骤表第 5 步「神煞修饰」改为「六神兽对用神/动爻取象」（frv 无神煞分析，消除 README 自相矛盾）。

## v2.0.2 (2026-09-10)

### 立币作废机制（同构 main v2.0.2）

- `scripts/paipan.py`：新增立币作废——`toss_coins()` 逐枚三态判定：每枚硬币落地那一刻由**同一次抽取**当场定出「正 / 反 / 立」，立币与正反同源同抽；任意一枚立起即抛 `CoinEdgeError`、整卦作废；单枚概率常量 `DEFAULT_EDGE_PROB = 1/6000`（0..9999 网格就近取整为 2/10000）。新增 `--edge-prob`（默认 1/6000，设 0 关闭）；退出码约定 0 成功 / 1 参数错误 / 3 立币作废；作废时在 `-o` 同目录写 `paipan_void.json`，不产出 `paipan_result.json`。
- `scripts/paipan.py`：摇卦随机源改造——默认（未指定 `--seed`）改用**时间随机**（`make_time_rng()`，纳秒时钟 `time.time_ns()` 异或操作系统物理熵 `os.urandom(8)` 播种），使卦象锁死在起卦此刻；`toss_coins()/generate_ygua()` 改为接收注入随机源。关闭立币（slots=0）时抽取序列与 v2.0.1 逐位一致，历史 `--seed` 卦象仍可复现。
- 设计意图（防后人误改）：立币**不做独立随机源**——每枚硬币落地那一刻由同一次抽取定三态，与正反共享 `toss_rng`；独立叠加在物理上不成立。已废弃的"正面一条流、立币另一条流"写法不得恢复。
- `SKILL.md`：新增铁律 17「立币作废」；第零步第 6 步补充逐枚立币三态与 `--edge-prob` 说明，新增「立币作废分支（退出码 3）」处理流程、固定用户话术与专用输出块；检查点增加退出码判定。
- `references/rule-index.md`：登记 `TL-17`。
- `scripts/test_gua_patterns.py`：新增「测试 10：立币作废」「测试 11：时间随机源」，测试类数由 11 增至 13。

## v2.0.1 (2026-09-01)

### 维护界面与发布治理升级

> 本版本不削弱规则，不重写断法，不改变排盘、格局检测、HTML schema 或第八步审查语义；目标是让强规则更好维护、更容易巡检、更不容易版本漂移。

- `SKILL.md`：版本升至 v2.0.1；新增「设计意图与读法」，说明本 Skill 是防弱 AI 出错型生产规程，不是轻量提示词。
- `SKILL.md`：全局铁律按维护视角分为运行协议、数据权威、判断完整性、审查协议、表达规范；保留铁律 1~16 的编号、规则本体和强制语气。
- `SKILL.md`：给铁律 6、8、11、13、14、15 增加「防错目的」，解释独立审查、落盘、格局深断、JSON patterns 单一权威、路由表优先、每步自检门禁分别防什么错。
- `SKILL.md`：步骤内强制加载提醒增加「执行复述 / 完整源点」标记，明确文末「参考资料加载规则」仍是加载条件的单一权威表。
- `README.md`：新增「维护者速览」和「发布前检查清单」，固定 Windows PowerShell 下的 UTF-8 验证命令，避免中文输出被 GBK 误判。
- `references/rule-index.md`：新增规则编号注册表，统一登记 TL 铁律与 PR 核心原则的定义源、相关文件、防错目的和维护备注。
- `scripts/check_release_consistency.py`：新增发布一致性检查脚本，以 `SKILL.md metadata.version` 为权威检查 README、CHANGELOG、HTML guide、`generate_report.py` 文件头和 CLI description。
- `scripts/generate_report.py` 与 `references/html-report-guide.md`：同步当前脚本版本展示到 v2.0.1，修复 CLI description 残余版本漂移。

## v1.8.4 (2026-09-02)

### 应期分层冲突修正（同构 main v1.9.4，frv 范围）

- `SKILL.md`：第六步强制加载 `yingqi-faze.md` 全文，修复“要求使用场景应期角色分层表但加载规则不保证读表”的冲突；第六步输出模板细化三层排序写法。
- `references/html-report-guide.md` 与 `scripts/generate_report.py`：新增 `step6.layers` 结构化字段，HTML 单独展示主应期、辅助节点、风险窗口，避免把三层排序塞进 `window/detail`。
- `scripts/generate_report.py --validate`：从全文关键词扫描改为按 `layers.main`、`layers.auxiliary`、`layers.risk` 分层校验；校验表仅覆盖本分支已支持 intent，不带入全功能版专属场景。
- `references/yingqi-faze.md`：明确场景表是检查清单而非死板取用；角色不现或不适用时必须在对应层说明理由，不可留空。

## v1.8.3 (2026-09-02)

### 应期分层能力泛化（同构 main v1.9.3，frv 范围）

- `references/yingqi-faze.md`：新增「场景应期角色分层表」，覆盖求财、官运、学业、感情、健康、出行、失物、词讼、天气、通用；不引入全功能版专属 intent。
- `SKILL.md` 与 `references/jie-gua-xiang-jie.md`：明确官运/跳槽只是样例，本分支所有已支持 intent 都要读取场景表；跨 intent 问法取并集，不得把辅助节点平铺成主窗口。
- `scripts/generate_report.py --validate`：从官运专项检查升级为按 intent 的角色分层检查；已知 intent 若漏掉关键角色层，会在生成前被拦截。
- `references/html-report-guide.md`、`README.md`：同步 v1.8.3 schema 与版本说明。

## v1.8.2 (2026-09-02)

### 特殊格局重点展示与应期排序修正（同构 main v1.9.2，frv 范围）

- `scripts/generate_report.py`：特殊格局深断与 `md_full` 全文附录支持 `**重点**` 安全渲染为加粗；特殊格局首页增加重点样式，避免整段深断没有视觉层级。
- `SKILL.md`：铁律 11 要求每个特殊格局最终断语标出一处重点；第六步新增「主应期 / 辅助节点 / 风险窗口」三层排序，避免把所有触发地支平铺为同等窗口。
- `references/jie-gua-xiang-jie.md`、`references/yingqi-faze.md`：补强用神临值月/日规则；官运/跳槽分清官鬼职位、父母 offer/合同、妻财待遇；不引入全功能版专属场景库。
- `references/html-report-guide.md`、`README.md`：同步 v1.8.2 展示与应期输出说明。

## v1.8.1 (2026-09-02)

### 特殊格局深断改造（同构 main v1.9.1，frv 范围）

1. **规则**：铁律 11 升级为「十项深断模板」，要求特殊格局交代来源、类型、参与者、角色归属、合冲/成局产物、力量状态、对用神、对世应、应期、结果断语。
2. **判法**：`references/di-zhi-relations.md` 补齐六合/六冲深断模板；`references/jie-gua-xiang-jie.md` 同步审查规则。保留 frv reference 边界，不引入 `zonghe-yingyong-leixiang.md`、`64-gua-yongfa.md`。
3. **HTML 与校验**：`scripts/generate_report.py` 的特殊格局板块优先展示 `step3.pattern` 人工深断文本，自动检测摘要只作兜底；`--validate` 拦截「合则牵缠」「冲则变动」等浅套话。

## v1.8.0 (2026-08-19)

### HTML 报告新增「旺相休囚死」能量展示板块（同构 main v1.9.0）

1. **爻象详表下方新增「旺相休囚死」板块**：按能量从高到低（旺 > 相 > 休 > 囚 > 死）展示五行之气在当前月令下的流转；数据由脚本从 `meta.yue_jian`（月建）自动推导，AI 组装 JSON 无需新增字段
2. **规则**（references/wuxing-shengke.md 第七节）：当令者旺，令生者相，生令者休，克令者囚，令克者死；月令五档寅卯木旺/巳午火旺/申酉金旺/亥子水旺/辰戌丑未土旺。例（申月）：金旺 水相 土休 火囚 木死，卡片按此能量序排列
3. **样式**：五张卡片统一浅宣纸背景，五行名与状态左右并排；五行字色低饱和（木青/火红/土黄褐/水黑/金白深描边保证浅底可见）
4. 文件：`scripts/generate_report.py`（CSS `wxs-*` + `build_wang_shuai_block` + 板块自检）+ `references/html-report-guide.md`（板块 3b2 + 验证清单）；版本号 v1.7.5 → v1.8.0

## v1.7.5 (2026-08-17)

### 第八步审查改单轮制 + 重点核查项（同构 main v1.8.5）

1. **单轮审查制**（铁律 6 + 第八步重写）：审查智能体只执行一轮——发现问题如实写入报告，主流程按报告问题清单回退对应步骤修正（修正落到步骤 md 文件），修正后不再启动复审，直接进入第九步；修正内容与未通过项在步骤8报告中显式记录，不掩盖问题。删除「复审循环」「复审上限 3 轮」段落；第九步准入条件同步（修正落盘即视为可交付）
2. **重点核查项**（审查 prompt + jie-gua-xiang-jie.md 第八节，缺查即审查无效）：
   - **动变路径（第三步）**：动爻逐爻遍历完整；每一动爻「变爻→本爻」五行方向验算（回头生/回头克/化进/化退标签与 JSON bian_wu_xing 逐对核对）；暗动/日破正确判定
   - **三合/三会局（第三步 + 第七步）**：三合/三会局（含缺字半合、日月补字、旬空打折）按铁律 11 四要素逐一分析，只引不析=漏判；报告自称「无某局」→ 回查 JSON 真伪；局际方向验算
   - **生克方向（全链路）**：用神↔月建日辰、世应、动爻↔用神、变爻↔本爻、刑害合冲每一处方向独立重算——方向反是最高频错误（实测：应克世误作世克应、水局克火局方向反均靠独立重算才抓获）
3. 版本号 v1.7.4 → v1.7.5

## v1.7.4 (2026-08-04)

### 特殊格局分析强化（同构 main v1.8.4）

1. **铁律11强化**（SKILL.md）：第七步综合断语对每个特殊格局必须逐一交代四要素（①成局/虚势状态 ②所强/所制之爻（六亲视角）③对用神吉凶 ④结果断语），禁止「只列不析」；第八步审查判据同步（只提及不分析 → 判⚠️漏判）
2. **三合/三会局补断点方法论**（SKILL.md + references/di-zhi-relations.md）：定局五行 → 所强六亲 → 所强之爻 → 对用神吉凶链路；第三步模板强约束；第八步审查新增**逆向思维证伪**硬性要求
3. **三合/三会细微区别补全**（references/di-zhi-relations.md）：贵派成局条件（一动+日月补齐）、半合子型、虚合/合绊、破局冲枢纽字、缺字应期、区别速查表
4. **三合/三会规则经真实卦例校验**（references/di-zhi-relations.md）：三合缺字可被变爻补全（解读层标注）、半合带旬空打折（动爻空不减力）、三会成员仅计主卦爻
5. **六冲关系描述细化**（scripts/generate_report.py）：旺相者冲而不散、休囚者逢冲易散，吉凶须参用神旺衰
6. **按真实卦例审查教训补强规则**（2026-08-17，卦例 `26.08.17-山雷颐-命运` / `26.08.17-天雷无妄之水雷屯-旅行` 的第八步智能体审查暴露的规则层缺口，同构 main v1.8.4 第 6 项）：
   - `SKILL.md` 第一步新增**忌神/原神点名**输出（B-8d：全流程从未点名忌神）
   - `SKILL.md` 第二步新增🚨空亡爻必须判旺衰——旺相空=不为真空（出旬即实）、休囚空才真空（C-15：旺相旬空被按「真空虚悬」处理）
   - `SKILL.md` 第六步应期完整性补强：空亡先判旺衰（出旬即实优先）、忌神/原神应期、伏神出伏具体日期（值伏日+冲飞日，飞神空亡→伏居空地事不待冲）（B-8a~d）
   - `SKILL.md` 第七步强制输出块补铁律15 清单位置（D-1：步骤7 完全缺失清单块判硬伤）
   - `references/jie-gua-xiang-jie.md` 第六节应期法则补出旬即实/忌神原神/伏神出伏；第八节 B8 判据补「月破」类别 + 空亡先判旺衰 + 飞神空亡标注；第七节模板补清单位置

## v1.7.3 (2026-08-03)

### 套餐徽章模板变量化 + 报告作者信息（同构 main v1.8.3）

1. **徽章文本模板变量化**：`generate_report.py` 改用 `{edition_badge}` / `{edition_class}` 变量，由 JSON `meta.edition` 驱动；本分支 SKILL.md 组装规则固定填「免费基础版」；`--validate` 校验 edition 合法性。两分支 `generate_report.py` 恢复逐字节一致
2. **报告作者信息移至报告顶部**（Cyrux）：HTML 标题区新增作者行「作者：Cyrux ｜ 微信：drdingno1 ｜ 邮箱：Cyrux.ding@outlook.sg」，页脚作者行移除
3. **`yue_kong` 必填说明修正**：月柱必有旬空，不得省略——最多与日空相同（日月同旬）也须单独填写（references/html-report-guide.md meta schema）
4. 文档：`html-report-guide.md` meta schema 新增 `edition` 字段；SKILL.md 第九步新增 2c 组装规则；版本号 v1.7.2 → v1.7.3

## v1.7.2 (2026-08-03)

### 术语统一「六神兽」+ 日空月空分离 + 卦象盘面（同构 main v1.8.2）

1. **术语统一**：frv 版「六兽」、main 版「六神」两种写法统一为「六神兽」（含步骤名「六神取象」→「六神兽取象」、落地文件名 `步骤5-六神兽取象.md`、jie-gua-xiang-jie.md 第五节「六神兽吉凶象表」）；代码标识符与 JSON 字段名不动；`paipan.py` 仅改注释（VALID_INTENTS 等 frv 差异保留）
2. **日空/月空分开展示**：meta schema 新增 `yue_kong`（月空）字段，`kong_wang` 明确为日空，禁止合并字符串；HTML 信息条拆为「日空」「月空」两个独立项（`split_kong_wang()` 兼容旧合并格式）
3. **「六爻盘面」→「卦象盘面」**：HTML 板块名与文档同步改名

## v1.7.1 (2026-08-03)

### HTML 报告全文保真 — md_full 解卦过程全文附录（同构 main v1.8.1）

**问题**：第九步组装 JSON 时，执行者把步骤 md 的丰富解卦逻辑与断语压缩成字段摘要（案例实测：步骤7 md 2907 字 → JSON 899 字），HTML 报告被大幅精简。

**方案**：JSON 新增 `md_full` 顶层字段逐字承载步骤 md 全文，HTML 末尾新增「📜 解卦过程全文」附录板块完整展示；同时强化组装规则并加机器校验兜底。

#### 改动

| 文件 | 改动 |
|------|------|
| `scripts/generate_report.py` | ① 渲染：HTML 末尾新增「📜 解卦过程全文」板块（md_full 逐字展示，pre-wrap）；② `--validate` 新增 md_full 保真校验（缺失/为空/长度低于下限 step1~6≥50 字符、step7≥200 字符/含占位文本 → 报错阻断）；③ 容错：md_full 缺失时附录板块自动隐藏；④ 占位 JSON 补 `md_full: {}` |
| `references/html-report-guide.md` | schema 新增 `md_full` 定义（全文保真铁则，沿用 frv「六兽」术语）；step7 字段说明强化为全文承载；生成后验证新增附录检查（11→12 板块） |
| `SKILL.md` | 第九步新增「保真组装规则」：md_full 必须逐字写入步骤 md 原文；`--validate` 说明补 md_full 检测；强制输出与检查点同步；版本号 v1.7.0 → v1.7.1 |

## v1.7.0 (2026-08-03)

### 智能体审查 — 断后校验从自检改为独立审查智能体

**问题**：第八步断后校验由解卦 Agent 自己执行 A/B/C/D 四项检查并写校验报告——"运动员兼裁判"。旧版第八步最终检查点「16条有无不诚实标记」已自认软肋：自检报告无外部监督，存在掩盖问题直接交付的风险。

**改造**：第八步「断后校验」→「智能体审查」。审查由独立审查智能体执行（Agent 工具启动 subagent，与解卦主流程上下文隔离），主流程只负责委派、接收报告、按报告修正、复审（上限 3 轮）。审查智能体只依据卦例目录落地文件（`步骤1-*.md`~`步骤7-*.md` + `paipan_result.json` patterns 权威数据）独立审查 A/B/C/D 四项，报告写入 `步骤8-断后校验.md`。第九步 `--validate` 机器校验层不变，与智能体审查互补。

#### 文件改动

| 文件 | 改动 |
|------|------|
| `SKILL.md` | 铁律 6「校验不可略」→「审查不可代」；第八步整节重写（审查智能体启动 prompt 模板 + 复审循环）；第九步准入条件同步；流程表第 8 行与 frontmatter 更新；版本号 v1.6.0 → v1.7.0 |
| `references/jie-gua-xiang-jie.md` | 第八节标注「校验规则由独立审查智能体执行，主流程不得自查代替」 |
| `CHANGELOG.md` | 本条目 |

### 修复悬空 references 引用（剥离遗留）

frv 剥离高级功能时删除了 3 个 reference 文件（`liuqin-liushen-leixiang.md`、`zonghe-yingyong-leixiang.md`、`64-gua-yongfa.md`），但 SKILL.md 的 intent 路由表与各步骤加载触发仍引用它们，会导致第八步审查 D 项（references 清单一致性）误报。本次按「references 中不存在的文件一律删除对应引用」原则清理：

- **intent 路由表**：删除 `liuqin`/`zonghe`/`64-gua-yongfa` 全部引用；「学业」「词讼」两行引用全悬空 → 整行删除；其余行保留现存文件引用（`wuxing-shengke` / `bagua-leixiang` / `shier-dizhi-leixiang` / `te-shu-ge-ju` / `64-gua` / `yingqi-faze`）
- **Tier 归类总表**：删除 5 行悬空条目（Tier A/C 的 liuqin 项、Tier E 的 zonghe/64-gua-yongfa 项）；Tier A/C 阶段说明改为指向 `jie-gua-xiang-jie.md` 第一节/第五节（frv 版用神识别与六神类象的承载文件）
- **跨 intent 示例**：改写为引用现存文件的示例（健康/出行）
- **步骤触发**：第四步 Tier C 触发、第五步"额外加载六神精细类象"、第七步 Tier E 触发与 zonghe 补读要求全部清理
- **加载速查表**：删除「intent 对应生活场景」「intent=阳宅」「六十四卦爻象/阳宅灶门」「六亲/六神精细类象」4 个悬空行

> ℹ️ 注：frv 的 CHANGELOG 自 v1.7.0 起与 main 版本号独立（frv 保留 main v1.7.x 的功能历史记录，版本号从 frv 1.6.0 起独立递增）。

## v1.7.2 (2026-07-22)

### 三会局状态机 v2 — 日月入组 + 旬空压制 + 四态判定

重写三会局判定逻辑。激活源：动爻 | 日月入组；旬空为压制因子，出空自动升级。引入四态状态机：

| 状态 | 条件 | 含义 |
|------|------|------|
| **严格成局** | 3动，或 2动+日月补第三字 | 力量整齐，主导全局 |
| **成局** | ≥1动无空亡，或 全静+日月入组+无旬空 | 有力 |
| **三会** | ≥1动+有空亡（动不空，空在它爻）→ 出空成局 | 暂时冻结，待出空升级 |
| **三会之势** | 全静+日月不在组，或 全静+日月入组+有旬空 | 有势无力，待时而发 |

- `paipan.py` `detect_patterns()` 新增四态判定 + `members` 字段逐爻标注 `is_dong`/`is_kong`/`is_sun_moon`
- JSON `patterns.sanhui_ju` 返回 `level`（等级）+ `members`（逐爻详情）+ `detail`（中文摘要）
- 三合局同步修正：日月占其中一字可代替动爻激活合局
- 示例：寅卯辰+1动(寅)+辰空 → 判定为「三会」，辰日出空自动升级为「成局」



### 六冲卦误判防护 + 应期措辞规范化

**问题**：Agent 在解卦时可能违反铁律 13，凭记忆手动判断六冲卦归属，导致风雷益等非六冲卦被误判。应期措辞中「冲动」模糊不清，旺相被冲应写「冲起」而非「冲动」。

#### 修复

| 文件 | 改动 |
|------|------|
| `SKILL.md` 铁律 13 | 新增 `ben_liuchong_gua=false` 时严禁六冲卦表述的反例警告，列出 5 个常见易错卦名 |
| `SKILL.md` 第三步 | `te-shu-ge-ju.md`「卦体结构格局」「常见六冲卦」「常见误判陷阱」三节改为**无条件加载**（修复 false 时裸奔漏洞） |
| `SKILL.md` 第八步 B 项 | 新增「格局↔JSON patterns」一致性校验行 |
| `SKILL.md` 第六步 | 新增措辞检查：旺相→冲起，休囚→冲破 |
| `references/te-shu-ge-ju.md` | 新增「常见误判陷阱」节，列风雷益等 6 个易错卦及 GUA64 表对照规则 |
| `references/yingqi-faze.md` | 补充「逢冲之日」冲起/冲破/冲动三级术语区分 |
| `references/dong-bian-fa-ze.md` | 暗动表格增加「冲起=暗动」「冲破=日破」术语别名 |

### generate_report.py 六冲卦检测修复

**问题**：`generate_report.py` `detect_patterns()` 用上下卦错卦互补关系（乾↔坤、震↔巽等）判定本卦是否为六冲卦，导致风雷益（上☴下☳）等非六冲卦被误标。

**修复**：改为六爻地支逐对相冲检测——初↔四、二↔五、三↔上三对全冲方为六冲卦（仅10卦）。

## v1.7.1 (2026-07-22)

### 防动变方向性错误 — 三层防线

修复 Agent 在判定动爻变化性质时可能把「本爻克变爻」误判为「回头克」的方向性 bug。根因是 Agent 凭直觉跳过了五行生克方向确认步骤。

#### 三层防线

| 层 | 位置 | 机制 |
|---|---|---|
| 1 | `references/jie-gua-xiang-jie.md` §3.2 | 新增「判定前置步骤」— 必须先写五行再查表判定 |
| 2 | `SKILL.md` 第三步 | 输出模板增加五行方向确认行 + 受控词表；检查点追加方向追问 |
| 3 | `scripts/generate_report.py` --validate | 机器校验：`DIZHI_WUXING` 表 + 方向比对，矛盾直接 exit(1) |

#### 受控词表

第三步 `change_type`：`回头生` / `回头克` / `化出我生者` / `化出我克者` / `化进神` / `化退神` / `化空` / `化破` / `化墓` / `化绝` / `化长生` / `化帝旺` / `化胎` / `化合`

#### 第八步新增校验

B 项新增第 9 条：`动变方向↔五行生克` — 逐爻验证生克标签方向一致性。

## v1.7.0 (2026-07-20)

### 太岁体系全面升级 — 从旺衰辅助到长期应期核心

太岁从单薄的"年运天花板"提升为按时间范围切换角色的立体维度，并新增神煞联合取象、月旬空补全等配套能力。

---

#### 一、太岁层旺衰（第二步）— 三层→四层

旺衰判定从「月建→日辰→特殊状态」升级为「**太岁→月建→日辰→特殊状态**」四层。

- 太岁生克权高于月建日辰，不直接计入 ±1 打分体系，但作为宏观判定维度凌驾于日月之上
- `references/jie-gua-xiang-jie.md` 第二节新增「太岁判定」小节，含速查表
- 太岁层级表：临用神（最强）> 生用神 > 比和 > 克用神（压制）

#### 二、太岁按时间范围切换角色（第零步 + 第二步 + 第六步）

**第零步新增反问**：Agent 在主动提问时询问用户时间范围（三个月 / 一年内 / 长期不限）。

| 时间范围 | 太岁角色 | 启用功能 |
|----------|----------|----------|
| 一年以内 | 旺衰天花板（辅助维度） | 仅太岁冲用神/冲爻时纳入，不递进 |
| 跨年/长期 | 应期核心坐标 | 逐级递进 + 强信号锁定 |

#### 三、太岁逐级递进法（第六步）— 长期应期核心工具

月建推月、日辰推日，太岁推年。跨年任务的应期以太岁为第一参考坐标：

**递进推断法**：克→生→临 链式递进。今年太岁克用（信号年）→ 明年生用（发力年）→ 后年临用（鼎盛年）。

**强信号锁定**（新增最高优先级）：当 **用神地支 + 驿马 + 同地支太岁年** 三者重叠时，为鼎盛中的鼎盛。如用神申金 + 驿马申 + 戊申年 → 三申共振。

#### 四、神煞联合取象（第五步）

paipan.py 已自动计算每爻 12+ 神煞（驿马、将星、天医、禄神、金舆、谋星、华盖、太极贵人、灾煞、天乙贵人等），此前未被使用。

- **联合公式**：「六神 + 神煞 + 六亲 + 地支 = 完整取象」
- **绑定原则**：神煞不独立断吉凶，必须落在具体爻位+六神+六亲上
- `references/jie-gua-xiang-jie.md` 第五节新增「神煞联合取象」小节 + 10 神煞速查表
- 第五步强制输出新增「神煞联合取象（逐爻）」+「全局神煞亮点」表格

#### 五、月旬空字段补全（第零步 + references）

此前 SKILL.md 只定义了 `kong_wang`（日旬空），完全遗漏 `yue_xunkong`（月旬空）。paipan.py 实际已计算但规范未覆盖。

- 第零步输出模板新增「日旬空」和「月旬空」两行
- 每爻 `kong_wang` 从 `true/false` 改为 `""/"日空"/"月空"/"日空月空"` 四态
- `references/dong-bian-fa-ze.md` 第二节、`references/jie-gua-xiang-jie.md` 第二节同步更新日月双空规则
- 日月可能同旬（空亡相同）也可能不同旬——各算各的，不叠加

#### 六、README 对比表更新

main vs frv 对比表新增两行差异化：
- **太岁层旺衰**：✅ 四层叠加 vs ❌ 三层
- **神煞联合取象**：✅ 12+神煞逐爻分析 vs ❌ 无

---

## v1.6.1 (2026-07-18)

### paipan 引擎 bug 修复 — 6 卦静态数据 + 三合三会规则对齐文档

**问题来源**：外部反馈 + 自检发现。修复 paipan.py 静态数据表与格局检测规则的多处偏差，使排盘引擎输出与权威文档（`di-zhi-relations.md`）一致。**v1.6.0 的 references 加载机制不受影响**。

**修复 1：GUA64 表 6 卦静态数据错误**

按标准纳甲筮法表核对，修正以下 6 卦的爻纳甲/世应位置（多数为下卦装错+世应错位双重 bug）：

| 卦 | 错误 | 修正 |
|---|---|---|
| 火山旅（c2） | 下卦装成坎宫纳甲（丑/亥） | 改回艮宫纳甲（辰/午/申），世应 1/4 正确 |
| 天火同人（c8） | 4 爻装成辰土（应为子孙丑土） | 改回丑土，世应 3/6 正确 |
| 地风升（d5） | 上卦装错且世应颠倒 | 改回震宫纯文（丑/亥/酉 × 2），世应 4/1 |
| 水风井（d6） | 下卦 1 爻装成辰土 | 改回丑土，世应 5/2 正确 |
| 坤为地（h1） | 世应颠倒（1/4） | 改为 6/3（八纯卦世在 6 爻） |
| 地雷复（h2） | 世应颠倒（6/3） | 改为 1/4（一世卦世在 1 爻） |

**修复 2：三会局（sanhui_ju）规则重写 — 3 档状态 + 动爻条件**

旧实现：仅判断"三支俱全+旬空"二元状态，违反文档定义。

新实现按 `di-zhi-relations.md` §三 三会局规则：

- **严格成局**：三支都动；或两支动 + 日月补第三字
- **三会之势**：三支俱全 + 含动爻但不满足严格条件；或三支全静但日月在该组（势而不局）
- **虚势**：三支俱全 + 全静 + 日月不在该组

**修复 3：新增三合局（sanhe_ju）检测 — 含日月引动与半合子型**

旧实现完全没有三合局检测，遗漏重要合局信息。

新实现按 `di-zhi-relations.md` §二 三合局规则，覆盖四种合局（申子辰水 / 亥卯未木 / 寅午戌火 / 巳酉丑金）：

- **成局**：三字全现 + 至少一动；或三字全静但日月占一字（**日月引动合局**——日月的作用）
- **虚合**：三字全现 + 全静 + 日月不在组内
- **半合**：二字现 + 含动爻（带 subtype：长生帝旺 / 帝旺墓库 / 长生墓库）
- **成局/虚合**（半合路径）：二字现 + 日月补第三字（视有无动爻分二档）
- **不记录**：二字现 + 全静 + 无日月补；或只 1 字现

**修复 4：detect_patterns 函数签名变更**

`detect_patterns(lines, ben_gua_attr, bian_gua_attr)` → 加 `month_branch="", ri_chen=""` 两参数，用于日月补字/日月引动判断。caller 同步更新传入 `gz["month_branch"]` 和 `gz["ri_zhi"]`。

**修复 5：JSON schema 新增 sanhe_ju 字段**

`patterns.sanhe_ju` 数组，元素含 `group`/`wuxing`/`status`/`subtype`/`missing`/`dong_positions`/`kong_positions`/`sun_moon_fills`。

**测试覆盖**：`test_gua_patterns.py` 扩展到 **10 类测试**：
- 测试 1B：6 卦静态数据正确性（GUA64 表）
- 测试 2：三会局 3 档状态（4 组 × 4 动爻组合 + 日月补字 + 反例，18 用例）
- 测试 2B：三合局 3 档 + 半合子型 + 日月引动（12 用例）
- 测试 3-7：地支六合/六冲/伏吟/反吟/独发独静（24 用例）
- 测试 8：build_paipan_data attr 拼接集成（4 卦）

**验证**：坤为地静卦（月未日巳）→ 亥卯未木 + 巳酉丑金 双双日月引动成局 ✓

**变更文件**：
- `scripts/paipan.py`：6 卦数据 + DIZHI_SANHE_GROUPS 常量 + sanhui_ju 重写 + sanhe_ju 新增块 + detect_patterns 签名 + caller
- `scripts/test_gua_patterns.py`：扩展到 10 类测试

---

## v1.6.0 (2026-07-18)

### references 加载机制深度修复 — Tier 分阶段预读 + 自检门禁防纸面合规 + 全方位测试覆盖

**庆祝 🎉**：本次是大版本更新，标志着 references 加载机制从「单点路由表 + 自检清单」演进为「分阶段精准预读 + 关键句摘录防伪 + 全方位自动化测试」。历经 4 轮逆向思维核查，从 1 项问题逐步扩展识别出 8 项 + 3 项遗留补修，全部闭环。

**v1.5.11 遗留问题盘点**（通过第三、四轮逆向思维发现）：

| # | 严重度 | 问题 | 根因 |
|---|---|---|---|
| 1 | 🔴 | 自检门禁是「纸面合规」—— Claude 可只写文件名不真读 | 清单格式未强制要求证明已读 |
| 2 | 🔴 | CHANGELOG 「零行为变更」误导 | 实际 JSON schema、铁律、校验都有扩展 |
| 3 | 🟡 | 早期步骤（2/3/4）不需要场景类 references，却被强行预读 | 第零步全量预读策略 token 浪费 |
| 4 | 🟡 | 「铁律 14/15」与「16 条核心原则 #14/#15」编号冲突 | 两套体系未做命名区分 |
| 5 | 🟡 | intent=通用 路由表只有 1 个 reference，兜底太弱 | 通用类被忽略 |
| 6 | 🟡 | 64-gua-yongfa 第六节阳宅内容古老（"产亡/夭寿/败绝"等极端断词） | 缺现代适配说明 |
| 7 | 🟢 | test_gua_patterns.py 只覆盖 GUA64 表 4 字段 | 三会/六合/六冲/伏吟/反吟/独发独静全未测 |
| 8 | 🟢 | attr 字符串拼接逻辑无集成测试 | 端到端验证缺失 |

**核心改动 1：分阶段预读（Tier A-E）替代全量预读**

| Tier | 触发时机 | 内容 | 用途 |
|---|---|---|---|
| A | 第零步附（立即） | `liuqin 第一节六亲基本类象` | 识别用神 |
| B | 第二步前 | `wuxing-shengke` / `yingqi-faze` | 旺衰判定 |
| C | 第四步前 | `liuqin 第三节~第七节`（六神各节） | 六神类象断语 |
| D | 第五步前 | `bagua` / `shier-dizhi` | 六神取象 |
| E | 第七步前 | `zonghe-yingyong-leixiang` / `64-gua-yongfa` / `te-shu-ge-ju` / `64-gua` | 场景结论 |

每个 reference 固定归属一个 Tier（见 SKILL.md「Tier 归类总表」）。第零步附只加载 Tier A，其他 Tier 在对应步骤的「🔌 Tier X 加载触发」块处加载。早期步骤不再浪费 token 加载暂不使用的场景类内容。

**核心改动 2：自检门禁防纸面合规（关键句摘录必查）**

旧版清单格式（易造假）：
```
- 本步已加载 references 清单：
  - [文件名 小节]
```

新版清单格式（强制证明已读）：
```
- 本步已加载 references 清单（含关键句摘录，证明已真读）：
  - [文件名 小节]：「[1-2 句核心内容摘录，如"乾卦身体对应首/肺，疾病对应头胸肺疾"]」
```

第八步 D 项校验新增「假合规检测」：清单项无摘录 → 判定为假合规 → 回退到该步重新真读后再摘录。

**核心改动 3：铁律命名规范**

- 全部 15 条铁律加「铁律 N·」前缀（如「铁律 1·排盘优先」）
- 在「全局执行铁律」节首加命名区分说明：「铁律 N」（执行流程约束）≠「16 条核心原则 N」（断卦推理原则）
- 全文清理「第 14 条铁律」与「铁律 14」混用，统一为「铁律 N」

**核心改动 4：intent=通用 路由扩展**

旧版：`通用 | liuqin 第一节六亲基本类象`（仅 1 项，兜底太弱）  
新版：`通用 | liuqin 第一节 + bagua 全文 + shier-dizhi 全文 + yingqi-faze`（4 项兜底）

**核心改动 5：64-gua-yongfa 第六节现代适配说明**

在「六十四卦阳宅——灶门(主)吉凶类象」节首加 ⚠️ 现代适配说明：
- 「门」≈ 单元门 + 入户门；「灶」≈ 厨房灶位；「主」≈ 主卧室
- 男女长幼定位按家中实际辈分调整，不机械套用古制
- 「应瑞/应凶年份」仅作参考，不作硬性预测
- 「产亡/夭寿/败绝」等极端断词理解为能量倾向，配合卦象整体旺衰综合判定

**核心改动 6：测试覆盖从 1 类扩展到 8 类**

| 测试类 | 覆盖范围 | 用例数 |
|---|---|---|
| 测试 1 | GUA64 表全量 you_hun/gui_hun/liuhe_gua/liuchong_gua（原有） | 64 |
| 测试 2 | 三会局 sanhui_ju（正例 4 + 缺支反例 + 旬空虚势） | 6 |
| 测试 3 | 地支六合 dizhi_liuhe（正例 6 + 全冲反例） | 7 |
| 测试 4 | 地支六冲 dizhi_liuchong（正例 6 + 全合反例） | 7 |
| 测试 5 | 伏吟 fuyin_positions（动变同支正例 + 异支反例） | 2 |
| 测试 6 | 反吟 fanyin_positions（动变相冲正例 + 相生反例） | 2 |
| 测试 7 | 独发 dufa / 独静 dujing（正例 + 4 个边界反例） | 6 |
| 测试 8 | build_paipan_data attr 拼接集成（游魂/归魂/六合/六冲 4 代表卦） | 4 |

总计 8 类 98 用例，全自动化执行。`python scripts/test_gua_patterns.py` 一键回归。

**核心改动 7：Tier 归类总表 + 步骤触发块**

- 在第零步附加「Tier 归类总表」：每个 reference 文件/小节固定归属一个 Tier
- 在第二/四/五/七步准入条件处加「🔌 Tier X 加载触发」块，明确加载时机
- 第八步 D 项校验加「分阶段一致性」判定

**CHANGELOG 描述校正**

v1.5.11 条目中「零行为变更」改为「变更范围澄清」，准确描述 JSON schema / intent 枚举 / 铁律 / 校验逻辑的扩展范围。

**验证**：

- `python scripts/test_gua_patterns.py` → ✅ 全部 8 类 98 用例通过
- `paipan.py --help` → ✅ 显示 13 类 intent（含寻人/阳宅）
- `paipan.py --subject "测试" --intent "求财" --seed 42` → ✅ JSON patterns 含 4 个新 bool 字段
- SKILL.md grep 检查 → ✅ 「铁律 N」命名统一，无「第 N 条」混用
- Tier A-E 结构 → ✅ 第零步附 + 第二/四/五/七步触发块齐全
- intent=通用 路由表 → ✅ 4 项兜底

**致谢**：用户的「逆向思维」方法论贯穿本轮修复——每一轮修复后用逆向思维反观，识别出 8 → 3 → 进一步补修的连锁问题。这种「修复 → 反观 → 再修复」的迭代节奏，是这次大版本质量保证的根本。

---

## v1.5.11 (2026-07-17)

### references 主动加载机制重构 — intent 路由表 + 自检门禁

**用户反馈**：「准确度已经非常高了，但是写了很多 reference，很多情况下都不会主动读取，只有寻物会主动读取类像」。

**逆向诊断**：原有「⛔ 强制加载」块措辞已为"必须"，但仍被跳过——根因不是路由不清晰，而是 **Claude 在十步长流程里"上下文疲劳"，越靠后越容易跳过读 reference 动作**。单纯加更大的路由表不够，必须配套自检门禁。

**核心改动**：
- **新增「第零步附：intent 路由表」**：13 个 intent 各自精确到 reference 小节的必读清单，在第零步意图确认后立即预读，贯穿后续步骤使用。支持跨 intent 问题取并集。
- **新增第 14 条铁律·路由表优先**：强制第零步预读。
- **新增第 15 条铁律·每步自检门禁**：每步开头必须显式输出「本步已加载 references 清单」才能继续。
- **第八步校验新增 D 项**：references 清单一致性检查，作为自检门禁的兜底。

**intent 表扩展 11 → 13 类**：
- 新增「寻人」（主用神：兄弟）—— 修复既有 bug（原第五步加载规则提到"寻人"但 intent 表没有，会被映射到"通用"绕过强触发）
- 新增「阳宅」（主用神：父母）—— 收编孤儿文件 `64-gua-yongfa.md` 第六节阳宅灶门断法
- 同步修改 `paipan.py` 第 16 行帮助文档 + 第 65 行 `VALID_INTENTS`

**paipan.py JSON schema 扩展**：
- `detect_patterns()` 新增 4 个 bool 字段输出：`ben_you_hun` / `ben_gui_hun` / `bian_you_hun` / `bian_gui_hun`
- 从 `ben_attr` / `bian_attr` 中文字符串提取（"游魂"/"归魂" in str），暴露为独立 bool 字段供 Agent 直接判定
- SKILL.md 第三步强加载块新增触发条件：上述字段任一为 true → 加载 `references/64-gua.md`

**三个孤儿文件按场景精准触发**（用户选择"按场景精准触发"策略，非流程必读）：
- `wuxing-shengke.md`（核查后非真孤儿——`jie-gua 第二节` 不含旺相休囚死表/六亲派生/纳音）：列入求财/健康 intent 路由表预读
- `64-gua.md`：第三步检测到游魂/归魂时强触发；阳宅 intent 路由表预读
- `64-gua-yongfa.md` 第五节：阳宅 intent 专属强触发

**健康类盲区补齐**：
- 第五步强加载条件从「失物/寻人/天气」扩展为「失物/寻人/天气/健康/阳宅」
- 健康 intent 在第零步路由表预读 `bagua-leixiang.md`（人体部位）+ `shier-dizhi-leixiang.md`（人体部位）用于定位病灶

**第七步措辞反向修正**：
- v1 计划曾把"加载"改成"复核引用"（弱指令）—— 若第零步预读被跳过，第七步连加载都不要求了，反而恶化
- v2 保留"加载"强指令 + 补读兜底："若未读则立即补读"

**场景类加载时机前移**：
- `zonghe-yingyong-leixiang.md` 原本只在第七步加载 → 现在第零步路由表预读，前 6 步分析时已有场景语境
- 第七步改为"确认已预读 + 复核引用"

**按需加载表标注升级**：
- `wuxing-shengke` / `64-gua` / `64-gua-yongfa` 三行加注"已升级为路由表/步骤强触发"，仅在 intent 未命中时按需

**token 成本**：单卦总占用约 80-100KB（用户确认在 1M 上下文模型下可接受）。

**验证**：
- `paipan.py --intent "寻人"` 不再报 invalid ✅
- `paipan.py --intent "阳宅"` 不再报 invalid ✅
- JSON patterns 含 `ben_you_hun`/`ben_gui_hun`/`bian_you_hun`/`bian_gui_hun` 字段 ✅
- 测试卦例（seed=42）本卦为游魂卦，`ben_you_hun=true` 正确识别 ✅

**变更范围澄清**：
- **算法核心不变**：旺衰分值规则、生克规则、应期法则、十步骨架全部保留
- **行为确有扩展**：
  - `paipan.py` JSON schema：新增 4 个 bool 字段（`ben_you_hun`/`ben_gui_hun`/`bian_you_hun`/`bian_gui_hun`）
  - `paipan.py` intent 枚举：11 → 13 类（新增"寻人"/"阳宅"）
  - SKILL.md 执行规则：新增第 14、15 条铁律（路由表优先 + 每步自检门禁含关键句摘录）
  - SKILL.md 校验逻辑：第八步新增 D 项（references 清单一致性 + 假合规检测）
  - 第三/五/七步加载条件、第零步流程（新增路由表预读环节）均有调整

---

## v1.5.10 (2026-07-15)

### 算法补遗 — 日月等权论述补入暗动机制

**盲点**：v1.3「日月等权」论述称月建与日辰在旺衰分值上「不分轻重」，但忽略了日辰独有的**暗动/日破**能力——日辰可冲静爻使之暗动（旺相/旬空时虽静实动）或日破（休囚时被冲散），月建无此机制。

**修正**：
- 澄清维度区分：**分值等权是生克维度**（日月 ±1），**暗动是冲动维度**（日辰独有）
- 更新三处核心文档：
  - `jie-gua-xiang-jie.md` 第二节提示框：补充日辰暗动/日破独有能力说明
  - `jie-gua-xiang-jie.md` 16 条核心原则第 4 条「日月如天」：补充暗动说明
  - `SKILL.md` 第八步速查表第 4 条：同步更新
- 零行为变更：旺衰分值规则不变，暗动/日破判定规则不变，仅修正论述层面的遗漏

## v1.5.9 (2026-07-10)

### 重构 — 拆分 `LUNAR_TABLE` 到独立 `lunar_data.py`

**动机**：v1.5.8 把 62 行农历查找表 `LUNAR_TABLE_2025_2086` 直接内嵌在 `paipan.py` 里，视觉上占屏让主脚本显得臃肿。用户提议"用立春+闰月等关键信息算法推算以减小体积"。

**调研结论**：算法路径不可行。
- 实测 2026-2086 全部 2196 天，用"正月初一锚点 + 平均朔望月 29.5306 天"推算，**错误率 77.5%**（1701/2196 天偏移）
- 根因：农历从清代《时宪历》起改为**定朔**（每月初一是实际新月日），朔望月长度在 29.18-29.93 天之间不规则波动，**无代数公式可算**
- 表本身其实很小：62 年 × 6 字节 = **372 字节 ≈ 0.36 KB**，已是最紧凑精确表示（位编码 1 bit/月）

**重构方案**：保持精确性，仅做代码组织优化。
- 新增 `scripts/lunar_data.py`：独立存放 `LUNAR_TABLE_2025_2086` + 详细数据结构/来源说明
- `paipan.py` 顶部改为 `from lunar_data import LUNAR_TABLE_2025_2086`
- 内部引用点（`_solar_to_lunar_pure()` 等共 5 处）零改动
- 文件头注释解释了"为什么不能用算法推算"（含 77.5% 错误率实验数据），防止未来又有人尝试

**验证**
- sxtwl 分支：`2026-07-10 13:54 → '丙午年五月廿六'` ✅
- pure 分支（`_SXTWL_AVAILABLE=False`）：同上 ✅
- 跨年：`2026-01-15 → '乙巳年十一月廿七'` ✅
- 闰月：`2028-07-05 → '戊申年闰五月十三'` ✅
- 端到端 `python paipan.py --subject ... --seed 42`：JSON `lunar` 字段正确写入 ✅

**零行为变更**：纯重构，外部 API、JSON schema、解卦流程均无变化。

## v1.5.8 (2026-07-10)

### 修复 — `lunar` 字段双轨自动输出（消除 AI 手算农历）

**问题**：`paipan.py` 已用 sxtwl 算四柱，但 **只输出公历 + 四柱，不输出农历日期**。HTML 报告 `meta.lunar` 是必填字段，迫使 AI 在生成报告时手算农历——既易错（实测解卦"找女友"时把五月廿六算成五月廿五，差 1 天）又浪费 token。

**修复 1 — `paipan.py` 双轨输出 `lunar` 字段**
- sxtwl 分支：直接调 `day_obj.getLunarYear/Month/Day()` + `isLunarLeap()`，零手算
- 纯 Python 分支：新增 `LUNAR_TABLE_2025_2086` 查找表（由 sxtwl 一次性预生成的 61 年数据，紧凑位编码：每月天数 1 位）+ `_solar_to_lunar_pure()` 查表函数
- 农历年干支：`_lunar_year_gz()` 优先查 `YEAR_GZ_LICHUN`，超出表用 60 甲子周期推算（支持 2026 年初跨年场景 → 乙巳年）
- `calc_ganzhi()` 两个分支 + `build_paipan_data()` 最终 result 均暴露 `lunar` 字段
- 覆盖 2025 是为了让 2026-01-xx ~ 2026-02-16（正月初一前）的占卜日期能正确归到农历乙巳年年末

**修复 2 — 纯 Python 回退月空计算既有 bug**（顺手）
- `calc_ganzhi_pure()` 第 373 行原有 `TIANGAN_INDEX[month_gan]`，但 `month_gan` 变量从未定义（只有 `month_gan_idx`）→ pure 模式必崩
- 因实际用户基本都装了 sxtwl，此 bug 从未暴露；本次加 lunar 字段触发测试才发现
- 修复：新增 `month_gan = TIANGAN[month_gan_idx]` 变量赋值

**验证**（9 个测试用例 × 双轨 = 18 次对比）
- 主流程：2026-07-10 → 丙午年五月廿六 ✅
- 正月初一：2026-02-17 → 丙午年正月初一 ✅
- 跨年（立春前）：2026-02-16 → 乙巳年腊月廿九 ✅
- 立春日：2026-02-04 → 乙巳年腊月十七 ✅
- 闰五月：2028-07-01 → 戊申年闰五月初九 ✅
- 闰十一月：2033-12-22 → 癸丑年闰十一月初一 ✅
- 中段：2050-06-15 → 庚午年四月廿六 ✅
- 表末：2086-02-14 / 2086-12-31 ✅
- sxtwl 与 pure 两后端输出完全一致

**文档**
- `SKILL.md` 第零步强制输出新增「农历日期」项，明确标注「**直接读 JSON `lunar` 字段，禁止手算**」
- `SKILL.md` 检查点关键字段列表新增 `lunar`
- `SKILL.md` frontmatter version `1.5.7` → `1.5.8`

## v1.5.7 (2026-07-09)

### 改进 — 排盘正确性、可复现性、流程重构

**修复 1 — 六神代码注释与实际逻辑矛盾**（`paipan.py` 第 829-831 行）
- 旧注释「自上而下排列：上爻(i=5)→初爻(i=0)」与代码 `(start + i) % 6` 实际的「自下而上」赋值方向矛盾，按注释"修正"会把六神全排反
- 新注释明确写出日干→起始六神映射 + 自下而上排列规则

**新增 — 摇卦 seed 可复现性**
- `paipan.py` 新增 `--seed N` 参数：指定后随机序列固定，同 seed 两次运行产出完全相同的 ygua
- JSON 顶层新增 `seed: int|null` 字段，便于事后复核"那天那个卦怎么排出来的"
- 占卜传统讲究"初筮告，再三渎"——seed 不改初筮随机性，但使卦象可追溯

**新增 — 特殊格局自动检测收敛到 `paipan.py`**
- 新增 `detect_patterns()` 函数 + 地支关系常量（六合/六冲/三会局对）
- JSON 新增 `patterns` 字段：`sanhui_ju` / `dizhi_liuhe` / `dizhi_liuchong` / `fuyin_positions` / `fanyin_positions` / `dufa` / `dujing` / `ben_liuchong_gua` / `ben_liuhe_gua` / `dong_count`
- SKILL.md 第三步从「AI 手动识别格局」改为「直接读 JSON patterns 字段」——单一权威，杜绝两套逻辑打架
- SKILL.md 全局铁律新增第 13 条「格局检测单一权威」

**新增 — `generate_report.py --validate` 子命令**
- 校验逻辑从 SKILL.md 的 inline Python heredoc 收敛到 `generate_report.py --validate`
- 检查 `meta`/`yao`/`steps` 顶层域 + `step1`~`step8` 齐全 + `step5` 四个六神字段 + 每爻 `ben_yin_yang`
- SKILL.md 第九步用 `python generate_report.py -i xxx.json --validate` 替代 heredoc——跨平台引号转义风险消除，schema 演进只需改一处

**改进 — 第零步交互轮次合并**
- 旧流程：问事 → 确认意图 → 检测 sxtwl → 问 A/B 选择 → 执行（3-4 轮 round-trip）
- 新流程：问事 → 同时呈现「意图确认 + sxtwl A/B 选择」→ 执行（2 轮）
- sxtwl 未安装时 ⚠️ **显眼标注**「B 纯 Python 回退仅支持 2026-2086 年」
- 纯 Python 年份越界的报错信息补充「请安装 sxtwl」提示

**改进 — 版本号与文档一致性**
- SKILL.md frontmatter `version: "1.5.0"` → `"1.5.7"`（修复 1.5.1-1.5.6 对外不可见的 bug）
- 流程标题「解卦流程（九步法）」→「解卦流程（十步法：第零步排盘 + 第一至九步解卦）」
- README 版本节更新
- CHANGELOG v1.5.3/4/5 重排为严格倒序

**改进 — 桌面路径回退不污染主目录**
- 旧回退：Linux 无 Desktop 时 → `~/`（卦例目录散落 home 根）
- 新回退：→ `~/.liuyao/cases/`（自动创建，隔离存放）

**改进 — 第八步内联 16 条原则速查表**
- 核心契约从 `references/jie-gua-xiang-jie.md` 第一节内联到 SKILL.md 第八步
- 校验时无需额外加载即可对照，减少 Agent 凭记忆偷懒的可能

### 变更文件
- `scripts/paipan.py`：六神注释修正；新增 `DIZHI_LIUHE_PAIRS` / `DIZHI_LIUCHONG_PAIRS` / `DIZHI_SANHUI_GROUPS` 常量；新增 `detect_patterns()` 函数；`build_paipan_data` 签名加 `seed` 参数；JSON 输出加 `patterns` 和 `seed` 字段；CLI 新增 `--seed` 参数；年份越界报错信息增强
- `scripts/generate_report.py`：新增 `validate_json()` 函数 + `--validate` 子命令；CLI 版本号 v1.4 → v1.5
- `SKILL.md`：frontmatter 版本号更新；角色定义与流程标题统一为十步法；全局铁律新增第 13 条；第零步交互合并 + sxtwl 警示 + seed 文档 + patterns 输出要求；第三步改为读 JSON patterns；强制触发条件表更新；第八步内联 16 条原则速查表；第九步用 `--validate` 替代 heredoc；桌面路径回退改为 `~/.liuyao/cases/`
- `README.md`：版本节更新为 v1.5.7

---

## v1.5.6 (2026-07-08)

### 改进 — 第零步文件管理重构 + 跨平台兼容

**问题**：排盘脚本 `paipan_result.json` 输出到用户主目录或当前工作目录，与桌面卦例目录分离。过程文件散落各处，缺乏统一管理。

**改进方案**：
- **全局铁律新增第12条**「文件统一存放」：所有过程文件必须统一存入桌面卦例目录，目录第零步创建、排盘后重命名
- **第零步流程重构**：意图确认后立即创建 `~/Desktop/YY.MM.DD-事件简称/` 临时目录 → 排盘 `-o` 输出到该目录 → 读取卦名后重命名为 `YY.MM.DD-本卦名-事件简称/`
- **第一步简化**：去掉「创建输出目录」块（已前移至第零步），改为确认目录存在
- **跨平台兼容**：桌面路径适配 Windows/macOS/Linux（含中文 locale 回退），重命名命令分平台说明（`mv` / `move`）
- **示例更新**：所有目录名示例统一使用「工作」场景

### 变更文件
- `SKILL.md`：全局铁律第12条、第零步步骤5-7重写、第一步简化、跨平台路径标注
- `.gitignore`：新增 `paipan_result.json` 忽略规则

---

## v1.5.5 (2026-07-01)

### 修复 — 第五步取象遗漏八卦/地支类象文件

**问题**：SKILL.md 将 bagua-leixiang.md（八卦万物类象）和 shier-dizhi-leixiang.md（十二地支类象）列在「按需加载」中。对于「失物」「寻人」「天气」等需要精确定位物品/人物类别的 intent，Agent 进入第五步取象时未加载这两个文件，凭记忆取象导致偏差。例如寻物卦中，离卦「中虚」（空腔容器）、辰土「盆坛」（罐形器皿）、午火「食物」三条类象均在文件中明列，但因未加载而遗漏，最终将空饮料罐误判为手机。

**修复方案**：
- SKILL.md 第五步强制加载规则新增：若 intent 为「失物」「寻人」「天气」等需精确定位物品/人物的类别 → 必须额外加载 references/bagua-leixiang.md 和 references/shier-dizhi-leixiang.md 全文，不得凭记忆取象
- 强制触发条件表新增一行：进入第五步且 intent 为失物/寻人/天气 → bagua-leixiang.md + shier-dizhi-leixiang.md（全文）

### 变更文件
- SKILL.md：第五步强制加载规则（第 350 行）+ 强制触发条件表（第 527 行）

---

## v1.5.4 (2026-07-01)

### 修复 — `ben_yin_yang` 字段缺失导致卦象面板显示错误

**问题**：HTML 报告中标题/信息条显示正确的卦名，但卦象面板（六爻符号 + 八卦符号）显示的是另一个卦。例如标题写「水山蹇」但面板画出「泽风大过」。

**根因**：`paipan.py` 输出的 JSON 中每爻缺少 `ben_yin_yang`（本卦阴阳属性）字段，`generate_report.py` 被迫从 `ygua`（原始六爻编码）推算。当 AI 在解卦流程中手工修改 JSON 数据时若忘记同步 `ygua`，推算结果就与 `ben_gua` 不一致，导致卦象面板分裂。

**修复方案**：
- `scripts/paipan.py`：`lines.append` 新增 `ben_yin_yang` 字段，基于 `gua[i]`（与 `guastr`/`ben_gua` 严格绑定）
- `scripts/generate_report.py`：扁平 JSON 适配器优先使用 paipan 直出的 `ben_yin_yang`，缺失时回退 `ygua` 推算（兼容旧 JSON）
- `scripts/generate_report.py`：`build_gua_panels` 新增 `ben_gua_name` 参数，面板卦名使用 JSON meta 的 `ben_gua`，与标题/信息条完全一致

### 修复 — 变卦非动爻缺少纳甲信息

**问题**：六爻详表中「变卦」列对非动爻显示的是本卦的六亲和地支（fallback），而非变卦自身的纳甲。例如风雷益→天地否，非动爻在变卦列应显示「子孙巳」，却错误显示本卦的「兄弟寅」。

**根因**：`paipan.py` 中 `bian_yao` 仅在 `is_dong` 为 `True` 时计算，非动爻的 `bian_yao` 为 `None`，导致 `generate_report.py` 用本卦数据作为 fallback。

**修复方案**：
- `scripts/paipan.py`：将条件从 `if is_dong and bian_gua_arr is not None` 改为 `if bian_gua_arr is not None`，只要存在变卦就为所有爻计算变卦纳甲（地支取自变卦结构，六亲按本卦宫五行重算）

### 变更文件
- `scripts/paipan.py`：新增 `ben_yin_yang` 字段（第 840 行）；bian_yao 计算扩展至全部爻（第 798 行）
- `scripts/generate_report.py`：扁平适配优先取直出字段（第 789-797 行）；面板卦名参数化（第 155、184、708 行）

---

## v1.5.3 (2026-06-25)

### 修复 — 终端编码导致排盘 JSON 乱码

**问题**：Windows PowerShell 中 `python paipan.py` 的 stdout 经过 Python（默认 GBK）→ PowerShell（UTF-8）双重编码转换后，中文全部变为乱码。此前 Agent 的做法是重新运行脚本（以 UTF-8 环境变量），但**重新运行会重新随机摇卦**，导致初筮之卦丢失——这是方法论错误。

**根因**：非终端问题，而是 Python 在 Windows 上未设 `PYTHONIOENCODING` 时默认走系统 locale（中文 GBK），与 PowerShell 的 UTF-8 编码打架。

**修复方案**（换终端不如绕过管道）：
- `scripts/paipan.py` 新增 `-o / --output` 参数：直接将 JSON 写入 UTF-8 文件，完全绕过 stdout 管道
- 文件模式下 stderr 输出简短确认信息，stdout 无中文
- SKILL.md 第零步第 5-6 条更新：**强制使用 `-o paipan_result.json` 文件模式**，然后用 `read_file` 工具读取（不经过 shell 管道）
- 新增 ⛔ 硬性规定：禁止依赖 stdout 传递中文 JSON

### 变更文件
- `scripts/paipan.py`：新增 `-o/--output` 参数（第 904 行、第 937-940 行、第 970-975 行）
- `SKILL.md`：第零步第 5-6 条重写，强制输出路径描述更新

---

## v1.5 (2026-06-08)

### 新增
- **自动排盘功能**：新增 `scripts/paipan.py` 自包含排盘脚本，合并 LiuYao 项目 `data.py` + `divination.py` 全部逻辑
- **第零步·自动排盘**：Skill 现在主动提问→分析意图/用神→三币摇卦→sxtwl 四柱计算→定卦→产出结构化 JSON，全程无需外部排盘系统
- **intent→用神速查表**：10 类意图（求财/官运/学业/感情/健康/出行/失物/词讼/天气/通用）的主用神与辅助用神一目了然
- **排盘脚本 CLI**：支持 `--subject`/`--intent`/`--yao`（手动六爻）/日期参数，三币随机或手动指定均可

### 变更
- 角色定义从「只解卦」升级为「排盘+解卦」
- 全局铁律新增第 0 条「排盘优先」
- 流程从九步扩展为十步（第零步+九步解卦）
- 输入格式说明更新：明确 JSON 结构由第零步产出
- README 新增「方式一：自动排盘」推荐用法

### 脚本细节
- `scripts/paipan.py`：826 行，零外部依赖（除可选 sxtwl），内嵌 64 卦 + 64 藏爻全部数据
- 三币摇卦概率：老阴(4)=1/8, 少阳(1)=3/8, 少阴(2)=3/8, 老阳(3)=1/8
- 输出 JSON 字段完整匹配 SKILL.md 输入格式：`date`/`ganzhi`/`month_branch`/`ri_chen`/`kong_wang`/`question`/`intent`/`ben_gua`/`bian_gua`/`lines[*]`（含 `fu_shen`/`bian_yao`/`liu_shou`），并带 `backend` 标记计算来源

### v1.5 后续补丁（同日）

**双轨干支计算**：
- sxtwl 改为可选依赖：已安装→`getYearGZ(False)` 立春分界；未安装→纯 Python 回退
- 纯 Python 路径：五虎遁（年干→月干）+ 五鼠遁（日干→时干）+ 日干支推算（2026-04-20 甲子基准）+ 旬空计算
- `YEAR_GZ_LICHUN` 嵌入 2026-2086 年干支 + 立春日期（用 `getJieQiByYear` 精确到日，立春分界）
- `_get_month_zhi()` 12 节气显式逐月判断，±1 天精度
- 年柱推算修复：`getYearGZ(True)`（春节）→ `getYearGZ(False)`（立春），与六爻传统一致

**第零步交互优化**：
- 意图确认后新增 sxtwl 检测步骤：已安装→直接排盘；未安装→询问用户选 A(安装) 或 B(纯 Python 回退)
- JSON 输出新增 `backend` 字段（`sxtwl` | `pure`），解卦时可据此判断时间精度

**子智能体验证**：
- 确认子智能体（DeepSeek Flash）不适用于本 skill：无 shell 执行能力、无法支撑解卦推理、压缩 reference 会丢失质量
- 结论：排盘+解卦全部由 V4 Pro 父进程单线程完成，不做子智能体分解

**旬空公式修复**：
- 原 LiuYao 项目 `divination.py` 旬空公式 bug：`cha += 10`（天干周期）→ 修正为 `cha += 12`（地支周期）
- 影响范围：所有 `日支序 - 日干序 < 0` 的日柱（癸日、壬日等），旬空全错
- 修复后与野鹤六爻排盘系统对照验证通过（癸丑日旬空寅卯）

**变卦六亲修复**：
- 从「变卦自身宫五行定六亲」改为「**本卦宫五行定变卦六亲**」（传统本卦宫法）
- 新增 `_GONG_WUXING`（八宫→五行）、`_liuqin_by_gong()` 函数
- 对照验证：兑宫归妹→变水解，初爻官鬼巳火 ×→ 妻财寅木（兑宫金克寅木=妻财，与野鹤六爻一致）

**神煞系统**：
- 新增 22 个神煞计算（`compute_shensha()`），按野鹤六爻 13 神煞标准对照验证
- 日干神煞（8个）：天乙贵人、禄神、羊刃、文昌、金舆、太极贵人、学堂、词馆
- 日支三合神煞（8个）：桃花、驿马、华盖、劫煞、灾煞、将星、亡神、谋星
- 月支神煞（4个）：天德、月德、天医（月支退一位）、天喜（按季节）
- 其他（2个）：天厨（日干）、干禄/日禄（别名）
- 贵人 bug 修复：庚日原误为丑未 → 修正为午寅（"庚辛逢马虎"）
- JSON 输出新增顶层 `shensha` 字段 + 每爻 `shensha` 数组（该爻地支命中的神煞名列表）

### v1.5 后续补丁 — HTML 报告 v1.4（同日）

**六爻盘面三栏+错综卦**：
- `generate_report.py` 升级至 v1.4（18.1 KB），内嵌 48 行八卦数据库 + 64 卦名字典
- **三栏盘面**：本卦 / 变卦 / 互卦 并排展示，各含卦名 + 六爻阴阳卦符（▅▅▅▅▅ / ▅▅  ▅▅）+ Unicode 八卦符号，无需纳甲信息
- **错卦/综卦按钮切换**：默认隐藏，点击按钮展开，面板仅展示卦名 + 卦符，无六神干支
- 错卦 = 阴阳全反（兑为泽 → 艮为山），综卦 = 上下颠倒（兑为泽 → 巽为风）
- **变卦完整显示**：脚本自动推算变卦完整六爻（非动爻保持本卦阴阳，动爻翻转），非动爻补全 `bian_yin_yang` 字段
- CSS 新增：`.gua-panels` / `.gua-panel` / `.gua-toggle-bar` / `.gua-toggle-btn` 组件，`.fu-yang`（赤）/ `.fu-yin`（青）卦符染色，打印模式自动展开错/综面板

**维护**：
- `html-report-guide.md` 标注脚本版本 v1.4

### v1.5 后续补丁 — JSON 数据录入防错（2026-06-09）

**Bug 修复 — 静卦 JSON 数据两个易错点**：
- **`yao[].ben_yin_yang` 缺失 → 卦象面板全错**：脚本 `ben_lines_from_yao()` 以 `y.get("ben_yin_yang") == "阳"` 判读每爻阴阳。AI 构建 JSON 时漏填此字段，`get()` 返回 `None` ≠ `"阳"`，六爻全判为阴爻 → 卦名误算为「坤为地」、卦符全部虚线。水火既济（阳-阴-阳-阴-阳-阴）的实际卦象被覆盖。
- **`bian_gua: null` → 渲染为 `"None"`**：静卦场景下变卦名没有实际值。Python `dict.get("bian_gua", "—")` 在 key **存在但值为 None** 时不走默认值路径，直接返回 `None`，被 `format()` 转为字符串 `"None"` 显示在标题和信息条中。静卦应传 `"静卦"` 字符串或不传此 key。

**规则澄清**：
- `ben_yin_yang` 是 `yao` 数组的**事实必填字段**（schema 标注为可选但脚本现实依赖），无论静卦动卦都必须为每爻填写 `"阳"` / `"阴"`
- 静卦 JSON 的 `bian_gua` 应设为 `"静卦"` 而非 `null`，确保 HTML 标题和信息条正确渲染
- 动卦正常传变卦名字符串（如 `"山风蛊"`），不存在此问题

### v1.5 后续补丁 — 脚本增强四合一（2026-06-09）

**A. 特殊格局自动检测**：
- `generate_report.py` 新增 `detect_patterns()` 函数，自动检测三会局（含严格成局/虚势/之势三级判定）、六合、六冲、本卦六冲卦、伏吟、反吟
- HTML 报告新增「特殊格局」板块，不同格局以不同色标渲染

**D. 神煞展示**：
- `generate_report.py` 新增 `build_shensha_section()` 函数，渲染顶层全局神煞 + 每爻神煞
- HTML 报告新增「神煞」板块，paipan.py 输出的 22 个神煞全部可视化

**E. 排盘增强**：
- paipan.py 新增 `--manual` 标志（配合 `--yao` 使用，JSON 中标记 mode: manual）
- paipan.py 新增 `--from-json` 参数（从已有排盘 JSON 导入，跳过摇卦直接标准化输出）
- generate_report.py 新增扁平格式自动适配器（paipan.py 直出的 lines 数组 → 自动包装为 meta/yao/steps）

**B. 解卦流程强化**：
- SKILL.md 全局铁律新增第 11 条「格局不遗漏」
- 第三步输出模板扩展：特殊格局从一行扩展为六项逐条检测（三会局/六合/六冲/本卦六冲卦/伏吟反吟/独发独静）
- 静卦也必须逐项检测，禁止空白或敷衍

---

## v1.4 (2026-06-01)

### 流程改进
- **每步落地到本地 md**：全局执行铁律新增第7条，每步「📤 强制输出」同步写入 `{卦例目录}/步骤N-名称.md`
- **第一步自动创建卦例目录**：桌面路径 `~/Desktop/YY.MM.DD-本卦之变卦-事件/`，后续所有产物（md、JSON、HTML）均归入此目录
- **第九步从本地 md 读取**：构建 JSON 前先遍历 `{卦例目录}/` 下的步骤文件，以本地文件为唯一数据源，不再依赖对话记忆
- **第九步新增 schema 校验**：JSON 写入后、脚本调用前，强制运行 `assert` 检查 `meta`/`yao`/`steps` 三大域及 `step5` 的子字段完整性
- **第三步新增「伏神旁注」子步骤**：用神明现但其爻下 `fu_shen` 非空时，执行一句话级别的伏神旁注，第七步综合断语必须引用

### 算法深化
- **日辰合绊三层解读**：新增 `dong-bian-fa-ze.md` 第六节，定下「现象→时效→寓意」三层递进框架
- **三会局认定严格化**：`di-zhi-relations.md` 第三节重写，三方须均在动爻/变爻中方为严格成局；否则降级为「三会之势」
- **伏神旁注规则表**：新增 `dong-bian-fa-ze.md` 第十节

### 维护
- `dong-bian-fa-ze.md` 全部标题重编号（五→六→…→十）
- `jie-gua-xiang-jie.md` 两处交叉引用更新（第八节→第九节）

---

## v1.3 (2026-05-21)

### 算法修正
- **日月等权**：月建分值从 ±2 改为 ±1，与日辰权重相等（`jie-gua-xiang-jie.md` 第二节）
- 月破/日破统一为 ±2（结构破损，重于普通克 ±1），月日破对等
- **合的保护**：新增规则——月建与用神有六合/半合/三合关系时，克的负面被保护抵消（死 → 0）
- 综合判定四档条件重写为总分制
- 明确「六爻测的是当下的能量状态」，月日同为此时此地的气，不分轻重；应期层面仍保持月管周期、日管急事的区分

### 新增
- 六爻盘面**卦符列**：本卦/变卦阴阳爻可视化（▅▅▅▅▅ / ▅▅  ▅▅），动爻展示本→变
- **特殊标签徽章**：旬空[空]、月破[破]、暗动[暗]、入墓[墓] 在爻表中直观标记
- **生克链条可视化**：chain-box 块，关键箭头和回头克关系染色高亮
- **趋势推演 + 行动建议**双栏布局：将综合断语拆分为客观趋势（绿）和可执行建议（橙）
- 综合断语中做了白话翻译，术语替换为生活语言

### 变更
- `generate_report.py` 完整重写（12.2 KB → 18.5 KB），新增 5 个 CSS 组件
- `html-report-guide.md` JSON schema 扩展：yao 新增 `ben_yin_yang`/`bian_yin_yang`/`special_tags`/`fu_shen`；step7 新增 `trend`/`action_advice`
- 容器最大宽度从 780px → 820px 适配卦符列

---

## v1.2 (2026-05-21)

### 变更
- 第九步 HTML 报告改为 **Python 脚本生成**（`scripts/generate_report.py`），AI 只整理 JSON 数据，不在对话中输出完整 HTML，大幅节省 tokens
- `references/html-report-guide.md`：新增 JSON 数据 schema（第二节），原有 HTML 模板降级为回退方案（第三节）
- SKILL.md 第九步：操作流程从「手动拼 HTML」改为「整理 JSON → 写文件 → 调用脚本」

### 新增
- `scripts/generate_report.py`：零依赖独立脚本，内嵌 HTML 模板 + CSS，读取 JSON 输出报告

### 不影响
- 前八步规则不动，JSON schema 字段完全对应前八步输出

---

## v1.1 (2026-05-21)

### 新增
- 第九步「生成 HTML 报告」：校验通过后自动生成可交付的网页版卦象报告
- `references/html-report-guide.md`：HTML 报告生成规范（结构模板、CSS 样式、验证规则）
- `README.md`：面向用户的 skill 说明文档

### 变更
- 流程从八步法升级为九步法
- SKILL.md 全局铁律、流程概览表、强制触发条件表、按需加载表同步更新
- 输出风格新增"HTML 报告排版美观、可直接交付问卦人"

### 不影响
- 前八步（取用神→校验）的所有规则、门禁、强制输出格式、检查点原封不动
- 全部 reference 文件的加载逻辑未变

---

## v1.0 (2026-05)

### 首次发布
- 八步法解卦流程：审题取用神、判定用神旺衰、追踪动变路径、伏神分析、世应关系、六神取象、应期推断、综合断语输出
- 第八步断后校验：步骤完整性 + 交叉验证 + 16 条原则映射
- 14 个 reference 文件覆盖解卦全知识域
- 12 类 intent 支持
- 半文半白输出风格
