...
- `scripts/paipan.py`：新增立币作废——`toss_coins()` 逐枚三态判定：每枚硬币落地那一刻由**同一次抽取**当场定出「正 / 反 / 立」，立币与正反同源同抽；任意一枚立起即抛 `CoinEdgeError`、整卦作废；单枚概率常量 `DEFAULT_EDGE_PROB = 1/6000`（0..9999 网格就近取整为 2/10000）。新增 `--edge-prob`（默认 1/6000，设 0 关闭）；退出码约定 0 成功 / 1 参数错误 / 2 孕产拦截 / 3 立币作废；作废时在 `-o` 同目录写 `paipan_void.json`，不产出 `paipan_result.json`。
