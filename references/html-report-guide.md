...
> 
> **脚本版本**：v1.7.2 — 卦象盘面（三栏本变互+错综切换）+ 日空/月空分开展示 + 解卦过程全文附录（md_full）。
...
| yue_kong | string |   | **月空**（月柱旬空地支），如 "辰巳"；无月空时留空/省略（v1.7.2：HTML 信息条「日空」「月空」分开展示） |
...
1. **结构完整性**：检查 12 个板块是否全部存在（含「📜 解卦过程全文」附录）
