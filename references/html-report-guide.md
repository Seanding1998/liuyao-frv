...
>
> **脚本版本**：v2.0.4 — 卦象盘面（三栏本变互+错综切换）+ 日空/月空分开展示 + 解卦过程全文附录（md_full）+ 旺相休囚死（五行之气月令流转）+ 特殊格局深断优先展示 + 重点加粗渲染 + frv 结构化应期场景分层校验 + 维护界面与发布一致性检查。
...
| edition | string | ✅ | **套餐版本标识**（v1.7.3）：frv 固定「免费基础版」；HTML 标题下套餐徽章据此渲染，禁止填错 |
...
| chain | string | 关键生克链条 |
| pattern | string | 特殊格局深断文本（铁律11十项深断：来源/类型/参与者/角色归属/合冲成局产物/力量状态/对用神/对世应/应期/结果）。HTML「特殊格局」板块优先展示此字段；自动检测摘要仅为兜底。禁止只写「合则牵缠」「冲则变动」等套话。每个格局最终断语用 `**...**` 标出 1 处重点，HTML 会渲染为加粗 |
...
| window | string | 综合时间窗口，只写总览，不替代 layers |
| unit | string | 日/月/年 |
| detail | string | 依据推算；须按 `yingqi-faze.md` 场景应期角色分层表覆盖当前 intent 的主用神本体、辅助条件、风险爻，不能平铺月份。官运/跳槽须分官鬼职位、父母 offer/合同、妻财待遇；其他 intent 同理 |
| layers | object | **必填**。结构化三层排序：`{"main": "...", "auxiliary": "...", "risk": "..."}`。main 写主应期，auxiliary 写流程/条件节点，risk 写阻力/破局/反复窗口；三项均不可空 |
...
<div class="step-full yingqi-layers">
  <p><strong>主应期：</strong>[直接落到所问之事]</p>
  <p><strong>辅助节点：</strong>[流程/条件]</p>
  <p><strong>风险窗口：</strong>[阻力/破局/反复]</p>
</div>
```
...
1. **结构完整性**：检查 12 个板块是否全部存在（含「📜 解卦过程全文」附录）；**另检查「旺相休囚死」板块（板块计数 13/13）**
