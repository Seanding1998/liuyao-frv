...
Agent 会确认意图，然后自动三币摇卦 → 排四柱 → 十步解卦 → 生成 HTML 报告。

---

#### 方式二：手动传入卦象（一句话排盘）⭐

你手里已经有卦了，不需要重新摇。比如朋友用硬币起了卦告诉你卦名和动爻，你直接一句话扔进去：

> 「火地晋，下个月工作升职调动有戏吗？动爻是三爻，2026年7月18日」

Agent 会先用你给的卦名和动爻定位卦象，补齐四柱、六亲、六神兽、世应，然后直接进入解卦流程。你只需要知道三样东西：卦名 + 动爻位置 + 日期，剩下的全交给它。
...
不用填表、不用记命令行参数，像聊天一样把卦象信息混在问题里说出来就行。
...
HTML 报告使用内联 CSS，无需外部依赖，可直接在浏览器打开、打印或分享。报告内含「旺相休囚死」板块（v1.8.0）：按能量从高到低展示五行之气在当前月令下的流转（如申月→金旺 水相 土休 火囚 木死）。
