...
Agent 会判定意图，然后自动**三币摇卦 → 排四柱 → 十步解卦 → 生成 HTML 报告**。

**软门禁·按需反问**：只有两种情况 Agent 会先问你一句（能一次问清就一次问完；其余情况直接排盘，不打断）：

1. **长周期问题 + 你没说时间** —— 如「我什么时候能升职」「以后发展怎么样」，Agent 会问你想看多久以内（三个月 / 一年内 / 长期不限）。
2. **意图判断不确定** —— 问题跨多个类别、指向不明时，Agent 会回读它的理解（意图类别 + 主用神），你直接说修正即可。

```
用户: 我以后事业怎么样？
Agent: 这件事你想看多久以内的情况？（如：三个月 / 一年内 / 长期不限）不指定的话，我按长期处理。
用户: 看三年吧
Agent: [排盘 → 输出卦象 → 进入九步解卦]
```

### 方式二：手动传入卦象（一句话排盘）

你手里已经有卦了，不需要重新摇。把**卦名 + 动爻位置（或变卦名）+ 日期**混在问题里一句话丢进去即可：

> 「火地晋，下个月工作升职调动有戏吗？动爻是三爻，2026年7月18日」

更复杂的也照样一句话：

> 「水山蹇变小过卦，问这次出差谈的合同能不能签下来」

> 「雷水解，静卦无动爻，看看我身体恢复得怎么样」

> 「天地否，动爻初爻和四爻，帮我断一下这个官司前景」

不用填表、不用记命令行参数 —— 像聊天一样说出来就行。

> 底层由排盘脚本的 `--gua-name` 配合 `--dong-yao`（动爻位置）或 `--bian-gua`（变卦名）确定性换算：报变卦名时，脚本对本卦与变卦逐爻取差自动得出动爻，**不靠人工比爻**。Agent 负责从你的话里提取卦名与动爻/变卦，换算交由脚本完成。
...
| 步骤 | 名称 | 说明 |
| --- | --- | --- |
| 0 | 自动排盘 | 提问 → 意图分析 → 三币摇卦（每枚落定正/反/立） → 四柱排盘 → JSON 卦象 |
...
- **自动排盘** — 三币摇卦法（每枚硬币落定正/反/立，出现立币即作废、隔日再占），sxtwl 双轨四柱，定卦 / 世应 / 六神兽 / 六亲 / 伏神全自动
...
- **与全功能版差异**：本分支聚焦核心解卦流程与通用场景断语；全功能版额外提供寻人、阳宅等更多意图场景、神煞联合取象、太岁三层旺衰与全量类象库 —— 详见 [全功能版介绍](https://github.com/Seanding1998/liuyao)
...
- **v2.0.6** — 修正六合卦纳支配对数据错误（同构 main v2.0.6）：`64-gua.md` 与 `te-shu-ge-ju.md` 的六合卦「原理」列所记爻位与真实纳支不符，致八行中六行配对错误（如泽水困误作「子亥合、寅丑合、辰巳合」，实为「寅亥合、辰酉合、午未合」；山火贲误作「辰酉合、午丑合」，实为「卯戌合、子丑合、寅亥合」）。已按 `paipan.py` 权威纳支重写两表并补「纳支（初→上）」列；`paipan.py` 检测逻辑与卦数据本就正确，本次仅修文档表述
- **v2.0.5** — 三合/三会成局口径校准（同构 main v2.0.5，用户明定）：统一激活源判据（动爻/日月入组），三会三态（严格成局[三爻全动·独有]／成局／三会之势）、三合三档（成局／半合／虚合，**无最高档**）；空亡不再作状态档，新增 `activation`/`void_trigger`（空字值日/值月即成局）
- **v2.0.4** — 第零步交互改按需反问（软门禁）：长周期问题未指定时间、或意图判断不确定时才回读反问，其余情况直接排盘不打断；反问与 sxtwl 选择合并一轮，无回应时按自判继续；强制输出块新增「时间范围」「意图把握」两行。
- **v2.0.3** — 审查修复与一句话手动排盘：移除误伤拦截词「生产」（保命的分娩类强特征词保留，求财问「生产项目」不再被拦）；新增 `--gua-name` / `--dong-yao` / `--bian-gua` 一句话手动排盘（本卦+变卦逐爻取差自动得动爻）；`ben_gua`/`bian_gua` 输出改纯卦名（去宫位前缀，宫位另见 `ben_gua_gong`）；补全退出码 0/1/2/3 处置规程（1/2 清理空目录、3 保留作废记录）；intent 路由表补「学业」「词讼」；统一 intent→用神权威口径；输入格式补 `sanhe_ju` 字段；HTML guide 补排盘 JSON→报告 JSON 字段转换表。
- **v2.0.2** — 立币作废机制：三币摇卦每枚硬币落定正/反/立（单枚约 1/6000），任意一枚立起即整卦作废（退出码 3），提示用户次日 0 点后再来；新增铁律 17 与第零步作废分支；摇卦随机源改为时间随机（纳秒时钟 × 物理熵）。
